...
public final class AnalysisAdvisor {
    ...
    private static DimensionSniff sniffFirstImageDimensions(final File directory, final File imageFile) {
        ...
        DimensionSniff result = runDimensionSniff(new Callable<DimensionSniff>() {
            @Override public DimensionSniff call() {
                return readDimensions(directory, imageFile);
            }
        }, DIMENSION_SNIFF_TIMEOUT_MS);
        DIMENSION_CACHE.put(key, result);
        return result;
    }

    private static DimensionSniff runDimensionSniff(
            Callable<DimensionSniff> task, long timeoutMillis) {
        ...
        Future<DimensionSniff> future = null;
        try {
    ...
    static DimensionSniff runDimensionSniffForTests(
            Callable<DimensionSniff> task, long timeoutMillis) {
        return runDimensionSniff(task, timeoutMillis);
    }
    ...
    private static DimensionSniff readProjectDimensions(File directory) {
        return readProjectDimensions(directory, new ProjectMetadataSource() {
            @Override public List<SeriesMeta> read(File projectDirectory) throws Throwable {
                return ImageSourceDispatcher.readAllMetadata(projectDirectory.getAbsolutePath());
            }
        });
    }

    static DimensionSniff readProjectDimensions(
            final File directory, final ProjectMetadataSource source) {
        return readDimensionSafely(new DimensionReadOperation() {
            @Override public DimensionSniff read() throws Throwable {
                List<SeriesMeta> metas = source.read(directory);
                if (metas != null) {
    ...
    private static DimensionSniff readFileDimensions(File imageFile) {
        return readFileDimensions(imageFile, new FileDimensionSource() {
    ...
    static DimensionSniff readFileDimensions(
            final File imageFile, final FileDimensionSource source) {
        if (imageFile == null || !imageFile.isFile()) {
            return DimensionSniff.noImage();
        }
        return readDimensionSafely(new DimensionReadOperation() {
            @Override public DimensionSniff read() throws Throwable {
                return DimensionSniff.confident(source.readZSize(imageFile));
            }
        });
    }

    private static DimensionSniff readDimensionSafely(DimensionReadOperation operation) {
        try {
    ...
    interface ProjectMetadataSource {
        List<SeriesMeta> read(File projectDirectory) throws Throwable;
    }

    interface FileDimensionSource {
        int readZSize(File sourceFile) throws Throwable;
    }

    private interface DimensionReadOperation {
        DimensionSniff read() throws Throwable;
    }

    static final class DimensionSniff {
        ...
        static DimensionSniff noImage() {
            return new DimensionSniff(false, false, 0);
        }

        static DimensionSniff uncertain() {
            return new DimensionSniff(true, false, 0);
        }

        static DimensionSniff confident(int zSlices) {
