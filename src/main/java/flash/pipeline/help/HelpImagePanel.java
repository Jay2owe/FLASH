...
public final class HelpImagePanel extends JPanel {
    ...
    public HelpImagePanel(AnalysisHelpTopic.HelpImage image) {
        this(image, MAX_IMAGE_WIDTH, MAX_IMAGE_HEIGHT);
    }

    public HelpImagePanel(AnalysisHelpTopic.HelpImage image, int maxWidth, int maxHeight) {
        ...
        JLabel imageLabel = loadImageLabel(resourcePath, maxWidth, maxHeight);
        imageLabel.setAlignmentX(Component.LEFT_ALIGNMENT);
        add(imageLabel);
        add(javax.swing.Box.createVerticalStrut(5));

        int captionWidth = Math.max(120, maxWidth - 20);
        JLabel captionLabel = new JLabel("<html><body width='" + captionWidth + "'>"
    ...
    private static JLabel loadImageLabel(String resourcePath, int maxWidth, int maxHeight) {
        ImageIcon icon = HelpImageLoader.loadScaledIcon(resourcePath, maxWidth, maxHeight);
        if (icon == null) {
            int placeholderHeight = Math.min(maxHeight, 90);
            ...
            placeholder.setPreferredSize(new Dimension(maxWidth, placeholderHeight));
            placeholder.setMinimumSize(new Dimension(Math.min(180, maxWidth), Math.min(70, placeholderHeight)));
            placeholder.setMaximumSize(new Dimension(maxWidth, placeholderHeight));
            return placeholder;
        }
