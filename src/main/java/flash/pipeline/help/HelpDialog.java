...
public final class HelpDialog {
    ...
    // Index-aligned to the analyses array in FLASH_Pipeline (0 = Set Up Configuration ...
    // 12 = Make Representative Image Figure). Keep this list in the same order.
    private static final String[] COMMON_PITFALLS = {
            ...
            "Slow on large stacks, and only reliable with good microscope metadata.",
            "Display only. Do not quantify from these enhanced images.",
            "Pick Volumetric or CPC colocalisation deliberately; segmentation quality drives the result.",
            "Re-runs from existing object CSVs; it does not re-segment images.",
            "Draw the line ROIs first, in Draw ROIs and Orientate Images, or it has nothing to measure.",
            "Mean and area-fraction give very different numbers; pick deliberately.",
            "Re-run after any per-image analysis change, and check condition labels first.",
            ...
            "Experimental. Over-correction removes real signal, so inspect previews and keep controls.",
            "For visual comparison only; the figure is not a new measurement."
    };
