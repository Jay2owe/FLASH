...
public final class HotspotScanAnalysis implements IntensitySpatialAnalysis {
    ...
    public static final String COLUMN_SEED = "Intensity_HotspotSeed";
    ...
    private static void measureInto(LinkedHashMap<String, Double> values,
        ...
        if (stats.varianceSum <= 0.0 || Double.isNaN(stats.varianceSum)
            ...
            values.put(COLUMN_SEED + suffix, Double.valueOf(Double.NaN));
            return;
        }
        ...
        long effectiveSeed = SpatialRandomSeeds.hotspot(context, !suffix.isEmpty());
        LocalStats localStats = localHotspotStats(grid, stats, context.config().getPermutations(),
                SpatialRandomSeeds.derive(effectiveSeed, "local"));
        double p = globalPermutationP(grid, stats, observedGlobal,
                context.config().getPermutations(),
                SpatialRandomSeeds.derive(effectiveSeed, "global"));
        ...
        values.put(COLUMN_SEED + suffix, context.config().getPermutations() > 0
                ? Double.valueOf((double) effectiveSeed)
    ...
    private static void addColumns(List<String> columns, String suffix) {
        ...
        columns.add(COLUMN_SEED + suffix);
    }
