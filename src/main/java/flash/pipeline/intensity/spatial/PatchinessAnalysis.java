...
public final class PatchinessAnalysis implements IntensitySpatialAnalysis {
    ...
    private static void measureInto(LinkedHashMap<String, Double> values,
        ...
        if (pixels.finiteCount == 0) {
            context.warn(giniSubject(suffix)
                    + " is undefined because the ROI contains no finite intensity values; "
                    + "returning NaN for patchiness metrics.");
            for (String column : columnsFor(context.config(), suffix)) {
                values.put(column, Double.valueOf(Double.NaN));
            }
            return;
        ...
        values.put("Intensity_PatchinessGini" + suffix,
                Double.valueOf(diagnosedGini(pixels, context, suffix)));
        for (double scale : scales) {
    ...
    private static double gini(double[] values) {
        ...
        // All-zero samples are perfectly equal, so their Gini coefficient is zero.
        if (sum == 0.0) return 0.0;
        double n = sorted.length;
        double result = (2.0 * weighted) / (n * sum) - (n + 1.0) / n;
        if (result < 0.0 && result >= -1e-12) return 0.0;
        if (result > 1.0 && result <= 1.0 + 1e-12) return 1.0;
        return result >= 0.0 && result <= 1.0 ? result : Double.NaN;
    }

    private static double diagnosedGini(PixelValues pixels,
                                        IntensitySpatialContext context,
                                        String suffix) {
        if (pixels.hasNonFinite) {
            context.warn(giniSubject(suffix)
                    + " is undefined because the ROI contains non-finite intensity values; "
                    + "returning NaN.");
            return Double.NaN;
        }
        if (pixels.hasNegative) {
            context.warn(giniSubject(suffix)
                    + " is undefined for negative intensity values; returning NaN without "
                    + "shifting or clipping the data.");
            return Double.NaN;
        }
        return gini(pixels.values);
    }

    private static String giniSubject(String suffix) {
        return suffix == null || suffix.isEmpty()
                ? "patchiness Gini"
                : "patchiness Gini for the binarized image";
    }
    ...
    private static final class PixelValues {
        ...
        final boolean hasNonFinite;

        private PixelValues(double[] values,
                            int sampleCount,
                            int finiteCount,
                            boolean hasNegative,
        ...
        static PixelValues from(ImagePlus image, int slice, Roi roi) {
            ...
            return new PixelValues(Arrays.copyOf(tmp, sampleCount), sampleCount, finiteCount,
                    hasNegative, hasNonFinite);
        }
    }
}
