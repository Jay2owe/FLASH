...
public final class MetricStatisticsEngine {
    ...
    public static List<StatisticRow> analyseMetric(
        ...
        boolean unpaired = cfg.pairedMode == StatisticsConfig.PairedMode.OFF;

        // One immutable finite-only sample set drives every unpaired decision
        // and result: eligibility, AUTO routing, tests, effects, confidence
        // intervals, post-hoc rows, and reported counts.
        Map<String, List<Double>> analysisGroups = unpaired
                ? finiteUnpairedGroups(conditionOrder, groups)
                : groups;
        if (unpaired) {
            String insufficientReason = insufficientGroupReason(conditionOrder, analysisGroups);
        ...
        if (cfg.distributionMode == StatisticsConfig.DistributionMode.ASSUME_NORMAL) {
            ...
            for (String cond : conditionOrder) {
                List<Double> vals = analysisGroups.get(cond);
                boolean normal;
                if (vals.size() < 8) {
                    normal = false;
        ...
        if (!unpaired) {
            return runPaired(metric, conditionOrder, analysisGroups, useParametric,
                    normalityInfo, cfg);
        }
        return runUnpaired(metric, conditionOrder, analysisGroups, useParametric,
                normalityInfo, cfg);
    }
    ...
    private static int totalN(Map<String, List<Double>> groups) {
        int n = 0;
    ...
    static double[] kruskalWallis(List<List<Double>> groups) {
        ...
        RankedSample ranked = rankWithTies(allValues);
        double[] ranks = ranked.ranks;
        ...
        H = (12.0 / ((double) N * (N + 1.0))) * H - 3.0 * (N + 1.0);

        double tieDenominator = (double) N * N * N - N;
        double tieFactor = tieDenominator == 0.0
                ? 0.0
                : 1.0 - ranked.tieSum / tieDenominator;
        if (tieFactor <= 0.0) {
            // Every observation has the same pooled rank: finite, neutral.
            return new double[]{0.0, 1.0};
        }
        H /= tieFactor;
        if (H < 0.0 && H > -1.0e-12) H = 0.0;
    ...
    static double[] mannWhitneyU(List<Double> a, List<Double> b) {
        ...
        double n1n2 = (double) n1 * n2;
        double meanU = n1n2 / 2.0;
        double tieAdjustment = N > 1
                ? ranked.tieSum / ((double) N * (N - 1.0))
                : 0.0;
        double varianceU = n1n2 / 12.0 * (N + 1.0 - tieAdjustment);

        if (varianceU <= 0.0) {
            // All-tied samples have U at its null mean and no separation.
            return new double[]{U, 1.0};
        }
        double stdU = Math.sqrt(varianceU);
    ...
    /** Mid-ranks plus the standard sum of {@code t^3-t} over tie blocks. */
    private static RankedSample rankWithTies(double[] values) {
        int n = values.length;
        ...
        while (i < n) {
            ...
            int tied = j - i + 1;
            if (tied > 1) {
                tieSum += (double) tied * tied * tied - tied;
            }
            i = j + 1;
        }
        return new RankedSample(ranks, tieSum);
    }
    ...
    static double chiSquaredPValue(double x, double df) {
        ...
        return 1.0 - new ChiSquaredDistribution(df).cumulativeProbability(x);
    }
    ...
    private static Map<String, List<Double>> finiteUnpairedGroups(
            List<String> conditionOrder,
            Map<String, List<Double>> groups) {
        LinkedHashMap<String, List<Double>> finite =
                new LinkedHashMap<String, List<Double>>();
        if (conditionOrder == null) return Collections.unmodifiableMap(finite);

        for (String condition : conditionOrder) {
            List<Double> source = groups == null ? null : groups.get(condition);
            ...
            finite.put(condition, Collections.unmodifiableList(clean));
        }
        return Collections.unmodifiableMap(finite);
    }
    ...
    private static final class RankedSample {
        ...
        private final double tieSum;

        private RankedSample(double[] ranks, double tieSum) {
            this.ranks = ranks;
            this.tieSum = tieSum;
        }
    }
