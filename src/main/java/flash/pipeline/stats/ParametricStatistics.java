...
public final class ParametricStatistics {
     ...
     * Equal constant samples are neutral. Unequal constant samples are
    ...
    public static double[] welchTTest(List<Double> a, List<Double> b) {
        ...
        if (se == 0.0) {
            return effect == 0.0
                    ? new double[]{0.0, 1.0}
                    : new double[]{Math.copySign(Double.POSITIVE_INFINITY, effect), 0.0};
        }
     ...
     * Equal constant groups are neutral. Constant groups with different
     * means are represented by the positive limiting statistic and a zero
     * p-value.
     *
