...
public final class PairedTests {
     ...
     * A non-zero constant difference is deterministic separation and is
     * represented by the signed limiting statistic and a zero p-value.
     *
    ...
    public static double[] pairedTTest(List<Double> a, List<Double> b) {
        ...
        if (variance == 0.0) {
            return mean == 0.0
                    ? new double[]{0.0, 1.0}
                    : new double[]{Math.copySign(Double.POSITIVE_INFINITY, mean), 0.0};
        }
     ...
     * A non-zero condition effect with zero residual error is deterministic
     * separation and is represented by a positive limiting statistic and a
     * zero p-value.
     *
    ...
    public static double[] repeatedMeasuresAnova(List<List<Double>> groups) {
        ...
        double sumOfSquaresScale = Math.max(1.0,
                Math.max(ssTotal, Math.max(ssConditions, ssSubjects)));
        double roundingTolerance = 32.0 * Math.ulp(sumOfSquaresScale);
        if (ssError <= roundingTolerance) {
            return ssConditions <= roundingTolerance
                    ? new double[]{0.0, 1.0}
