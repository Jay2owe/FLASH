...
public final class TukeyHSD {
     ...
     *         group with n &lt; 3 are dropped silently. With zero pooled
     *         within-group variance, equal means produce {@code q=0, p=1}
     *         and unequal means produce the limiting {@code q=+Infinity,
     *         p=0} deterministic-separation result.
     */
    public static List<PostHocRow> pairwise(List<List<Double>> groups,
        ...
        int dfWithin = N - k;
        if (dfWithin <= 0) return Collections.emptyList();
        ...
        for (int i = 0; i < k; i++) {
            for (int j = i + 1; j < k; j++) {
                ...
                if (se == 0.0) {
                    double meanDifference = Math.abs(means[i] - means[j]);
                    rows.add(new PostHocRow(conditionNames.get(i),
                            conditionNames.get(j),
                            meanDifference == 0.0 ? 0.0 : Double.POSITIVE_INFINITY,
                            meanDifference == 0.0 ? 1.0 : 0.0));
                    continue;
                }
