    ...
    public static boolean show(String directory, Set<BinField> fields) {
        ...
        while (true) {
            ...
            try {
                ...
                // fromBinConfig has no deconv projection; preserve any configured deconvolution from
                // the existing config so the direct-entry bypass never silently clears it.
                flash.pipeline.deconv.routing.DeconvConfigBridge.preserveDeconv(next, ChannelConfigIO.read(settingsDir));
                ChannelConfigIO.write(settingsDir, next);
