...
/**
 * Mask-restricted native-3D Grey-Level Co-occurrence Matrix features for one object.
 *
 * <p>The 13 unique half-sphere directions through the 26-neighbourhood are summed
 * into one symmetric matrix before Haralick features are computed. The offset
 * distance is one voxel in each participating axis. For anisotropic stacks this
 * deliberately uses the one-voxel neighbourhood rather than resampling or changing
 * z offsets; {@link Result#reliable} therefore reports statistical reliability,
 * not physical-isotropy equivalence.
 */
public final class ObjectTextureGLCM3D {
    public static final int DEFAULT_LEVELS = 32;
    public static final int DEFAULT_DISTANCE = 1;
    public static final int MAX_VOXELS = 256 * 256 * 256;
    public static final int VOXEL_CAP_SENTINEL_PAIRS = -1;
    ...
    private ObjectTextureGLCM3D() {
    }

    /**
     * Haralick feature result. A {@code coOccurrencePairs} value of
     * {@link #VOXEL_CAP_SENTINEL_PAIRS} means the input exceeded the 256^3 voxel
     * safety cap and feature values are NaN.
     */
    ...
    public static Result compute(ObjectPatch3D patch) {
        return compute(patch, DEFAULT_LEVELS, DEFAULT_DISTANCE);
    }

    public static Result compute(ObjectPatch3D patch, int levels, int distance) {
        if (patch == null) {
        ...
        if (patch.intensity.length > MAX_VOXELS) {
            return invalid(VOXEL_CAP_SENTINEL_PAIRS);
        }
        ...
        boolean valid = true;
        boolean reliable = pairs >= 16 && occupiedLevels >= 2 && hasCorrelation;
        return new Result(contrast, asm, correlation, entropy, homogeneity,
                valid, reliable, pairs);
    }

    private static int buildMatrix(ObjectPatch3D patch, int[] quantized, double[][] matrix, int distance) {
        int pairs = 0;
        int slice = patch.width * patch.height;
        for (int o = 0; o < OFFSETS.length; o++) {
            int dx = OFFSETS[o][0] * distance;
            int dy = OFFSETS[o][1] * distance;
            int dz = OFFSETS[o][2] * distance;
            for (int z = 0; z < patch.depth; z++) {
                int zz = z + dz;
                if (zz < 0 || zz >= patch.depth) continue;
                for (int y = 0; y < patch.height; y++) {
                    ...
                    for (int x = 0; x < patch.width; x++) {
                        ...
                        int aIndex = z * slice + y * patch.width + x;
                        int bIndex = zz * slice + yy * patch.width + xx;
    ...
    private static Range intensityRange(ObjectPatch3D patch) {
