...
public final class StatisticsConfig {
    ...
    /**
     * Whether observations are paired. {@link #WITHIN_ANIMAL} pairs repeated
     * sections/images of the same animal across conditions (the only pairing
     * design the engine supports). The legacy hemisphere/region/session split
     * was collapsed into this single within-animal mode; {@code region} and
     * {@code session} are still accepted as input synonyms.
     */
    public enum PairedMode {
        ...
        public static PairedMode parse(String value, PairedMode fallback) {
            ...
            if ("WITHIN".equals(normalized) || "WITHIN_ANIMAL".equals(normalized)
                    || "PAIRED".equals(normalized) || "IMAGE".equals(normalized)
                    || "SECTION".equals(normalized) || "SECTIONS".equals(normalized)
                    || "REPEATED".equals(normalized)
                    || "REGION".equals(normalized) || "SESSION".equals(normalized)) {
                return WITHIN_ANIMAL;
            }
            try {
