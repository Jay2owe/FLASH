...
public final class SpatialSetupConfig {
    ...
    public static final class DerivedConfig {
        ...
        /** Marker channel for the "resolve fused objects" overlap-count pass ("None"/empty = off). */
        public String clusterMarkerChannel = "None";
        /** Per-channel targets for the overlap-count pass (channel -> count marker centroids inside). */
        ...
        /** True when a marker channel (other than "None") is chosen and at least one target is enabled. */
        public boolean overlapCountOn() {
            if (clusterMarkerChannel == null) return false;
            String m = clusterMarkerChannel.trim();
            if (m.isEmpty() || "None".equals(m)) return false;
            for (Map.Entry<String, Boolean> e : clusterTargets.entrySet()) {
                if (Boolean.TRUE.equals(e.getValue()) && !m.equals(e.getKey())) return true;
            }
            return false;
        }
    }
