...
public final class ChannelSetupSupport {
    ...
    public static DerivedConfig deriveConfig(MetadataDiagnostics.SeriesInfo info,
        ...
        for (int c = 0; c < channels; c++) {
            ...
            if (entry == null) {
                // markerId is an explicit stable-library choice in this path;
                // accepting a missing ID would fabricate biological identity.
                throw new IllegalArgumentException(
                        "Marker library does not contain the selected marker ID '"
                                + markerId + "' for channel " + (c + 1) + ".");
            }
    ...
    /**
     * Resolves text accepted by the production channel-identity field to one
     * stable marker-library entry. Prefix and substring search hits are useful
     * suggestions, but are not safe identities to persist.
     *
     * <p>A non-matching name is a legitimate free-form channel and returns an
     * unbound result. If the channel previously claimed a marker ID that no
     * longer exists, the stale identity is reported instead of being copied
     * forward or silently converted to free-form.</p>
     */
    public static MarkerBinding resolveAcceptedMarker(String acceptedText,
                                                       String previousMarkerId,
                                                       MarkerLibrary library) {
        return resolveAcceptedMarker(acceptedText, null, previousMarkerId, library);
    }

    /**
     * Variant used by copied/filtered production state. An unchanged custom
     * display label retains its already-validated stable marker ID; editing the
     * label to new free-form text explicitly unbinds it.
     */
    public static MarkerBinding resolveAcceptedMarker(String acceptedText,
                                                       String previousDisplayText,
                                                       String previousMarkerId,
                                                       MarkerLibrary library) {
        String previous = safe(previousMarkerId).trim();
        if (library == null) {
            if (!previous.isEmpty()) {
                throw new IllegalArgumentException(
            ...
            return MarkerBinding.unbound();
        }

        MarkerLibrary.Entry previousEntry = null;
        if (!previous.isEmpty()) {
            previousEntry = library.byId(previous);
            if (previousEntry == null) {
                throw new IllegalArgumentException(
                        "Marker library does not contain saved marker ID '" + previous + "'.");
            }
            if (previousDisplayText != null
                    && safe(acceptedText).trim().equals(safe(previousDisplayText).trim())) {
                return MarkerBinding.bound(previousEntry);
            }
        }

        List<MarkerLibrary.Entry> exact = new ArrayList<MarkerLibrary.Entry>();
        for (MarkerLibrary.Entry entry : library.entries()) {
            if (entry != null && MarkerSearchRanking.isExactMarkerMatch(
                    entry.getId(), entry.getDisplayName(), entry.getAliases(), acceptedText)) {
                exact.add(entry);
            }
        }
        Collections.sort(exact, new Comparator<MarkerLibrary.Entry>() {
            @Override public int compare(MarkerLibrary.Entry left, MarkerLibrary.Entry right) {
                return left.getId().compareTo(right.getId());
            }
        });
        if (exact.size() > 1) {
            List<String> ids = new ArrayList<String>();
            for (MarkerLibrary.Entry entry : exact) ids.add(entry.getId());
            throw new IllegalArgumentException(
                    "Marker name '" + safe(acceptedText).trim()
                            + "' is ambiguous in the marker library: " + ids + ".");
        }
        if (exact.size() == 1) {
            return MarkerBinding.bound(exact.get(0));
        }
    ...
    /** Stable marker metadata copied into the persisted channel configuration. */
    public static final class MarkerBinding {
        public final String markerId;
        public final String markerShape;
        public final boolean crowdingSensitive;

        private MarkerBinding(String markerId, String markerShape, boolean crowdingSensitive) {
            this.markerId = safe(markerId).trim();
            this.markerShape = safe(markerShape).trim();
        ...
        private static MarkerBinding bound(MarkerLibrary.Entry entry) {
            return new MarkerBinding(entry.getId(), entry.getShape(), entry.isCrowdingSensitive());
        }

        private static MarkerBinding unbound() {
            return new MarkerBinding("", "", false);
        }
    }
