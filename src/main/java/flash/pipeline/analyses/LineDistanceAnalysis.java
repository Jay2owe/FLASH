...
public class LineDistanceAnalysis implements Analysis, RunRecordAware {
    ...
    private ChannelCsvPublisher channelCsvPublisher = new ChannelCsvPublisher() {
        @Override
        public void publish(File target, ChannelData data, String runId) throws IOException {
            CsvTableIO.writeChannelCsvChecked(target, data, runId);
        }
    };

    /** Checked required-output seam; production delegates to the durable CSV primitive. */
    interface ChannelCsvPublisher {
        void publish(File target, ChannelData data, String runId) throws IOException;
    }
    ...
    void setChannelCsvPublisherForTests(ChannelCsvPublisher publisher) {
        if (publisher == null) {
            throw new IllegalArgumentException("channel CSV publisher must not be null");
        }
        this.channelCsvPublisher = publisher;
    }
    ...
    public void computeDistances(String directory, File linesDir,
        ...
        for (Map.Entry<String, ChannelData> entry : channels.entrySet()) {
            ...
            try {
                channelCsvPublisher.publish(outFile, cd, currentRunId());
            } catch (IOException failure) {
                String message = "Required line-distance measurement publication failed for "
                        + outFile.getAbsolutePath();
                recordError(message, failure);
                throw new UncheckedIOException(message, failure);
            }
