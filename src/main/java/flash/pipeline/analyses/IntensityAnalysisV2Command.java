...
@Plugin(type = Command.class, headless = true, visible = false,
        name = "flash.intensityAnalysisV2",
        label = "FLASH Fluorescence Intensity Analysis")
public final class IntensityAnalysisV2Command implements Command {
    ...
    public void run() {
        ...
        final IntensityPreset preset = resolvePreset(projectDir);
        ...
        coordinator().run(analysis, FLASH_Pipeline.IDX_INTENSITY,
                "Fluorescence Intensity Analysis", projectDir.getAbsolutePath(),
                null, commandParameters(preset), empty(parentRunId),
                new Callable<Void>() {
    ...
    private IntensityPreset resolvePreset(File projectDir) {
        if (hasText(presetJson)) {
            try {
                return IntensityPreset.fromJson(presetJson);
            } catch (IOException e) {
                throw new IllegalArgumentException("Invalid presetJson for Fluorescence Intensity Analysis: "
                        + e.getMessage(), e);
            }
        }
        if (hasText(presetName)) {
            try {
                return new IntensityPresetIO(projectDir).load(presetName.trim());
            } catch (IOException e) {
                throw new IllegalArgumentException("Could not load IntensityPreset '"
    ...
    private Map<String, Object> commandParameters(IntensityPreset preset) {
