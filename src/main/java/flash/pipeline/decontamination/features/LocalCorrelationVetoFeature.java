...
public class LocalCorrelationVetoFeature implements CorrectionPipeline.ExecutableFeature {
    ...
    public void apply(CorrectionPipeline.ExecutionState state) {
        ...
        if (existingVeto != null) {
            ImagePlus merged = mergeVetoMasks(existingVeto, vetoMask);
            // mergeVetoMasks returns a NEW image; release the intermediate local-correlation veto
            // that is no longer referenced (the merged result replaces it).
            closeQuietly(vetoMask);
            vetoMask = merged;
        }
