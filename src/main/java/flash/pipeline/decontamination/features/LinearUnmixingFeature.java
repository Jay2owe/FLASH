...
public class LinearUnmixingFeature implements CorrectionPipeline.ExecutableFeature {
    ...
    private static final String SETTING_WEIGHT_SCALE = "weight_scale";
    ...
    public void apply(CorrectionPipeline.ExecutionState state) {
        ...
        // Subtraction-strength multiplier: scales every (fitted or manual) weight before the
        // per-pixel subtraction. Default 1.0 leaves the fitted result untouched. Re-sanitize so
        // the weight >= 0 invariant survives a negative scale.
        double weightScale = settings.getWeightScale();
        if (weightScale != 1.0) {
            for (int i = 0; i < fit.weights.length; i++) {
                fit.weights[i] = sanitizeWeight(fit.weights[i] * weightScale);
            }
        }
        ...
        CorrectionPipeline.FeatureSummary summary = new CorrectionPipeline.FeatureSummary(ID, getDisplayName())
                ...
                .putDouble("weight_scale", settings.getWeightScale())
    ...
    public static final class Settings {
        ...
        public Settings setWeightScale(double weightScale) {
        ...
        public CorrectionPipeline.Settings toPipelineSettings() {
            CorrectionPipeline.Settings raw = new CorrectionPipeline.Settings()
                    ...
                    .putDouble(SETTING_WEIGHT_SCALE, weightScale);
        ...
        public static Settings from(CorrectionPipeline.Settings raw) {
            ...
            settings.setWeightScale(raw.getDouble(SETTING_WEIGHT_SCALE, settings.weightScale));
