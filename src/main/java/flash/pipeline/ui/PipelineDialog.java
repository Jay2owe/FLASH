...
import flash.pipeline.help.ControlsHelpCatalog;
...
public class PipelineDialog {
    ...
    /** Routes title-bar and Escape cancellation through an owning wrapper. */
    ...
    /**
     * Analysis index of the single "?" help header on this dialog, or -1 if
     * none. Used to auto-attach per-control hover help from
     * {@link ControlsHelpCatalog} without adding extra "?" icons.
     */
    private int analysisHelpIndex = -1;

    /**
     * Attaches per-control hover help (a rich tooltip box) to the given widgets
     * if this dialog has an analysis help header and the control's on-screen
     * label is documented. Silently does nothing otherwise, so call sites never
     * need to know whether help content exists.
     */
    private void attachControlHelp(String label, JComponent... targets) {
        if (analysisHelpIndex < 0 || label == null) {
            return;
        }
        String tip = ControlsHelpCatalog.tooltipForControl(analysisHelpIndex, label);
        if (tip == null) {
            return;
        }
        for (JComponent target : targets) {
            if (target != null) {
                target.setToolTipText(tip);
            }
        }
    }
    ...
    public JButton addAnalysisHelpHeader(String text, int analysisIndex) {
        this.analysisHelpIndex = analysisIndex;
    ...
    public ToggleSwitch addToggle(String label, boolean defaultValue) {
        ...
        row.add(Box.createHorizontalGlue());
        row.add(toggle);
        attachControlHelp(label, row, lbl, toggle);
    ...
    public ToggleSwitch addToggleWithStatus(String label, boolean defaultValue,
        ...
        attachControlHelp(label, lbl, toggle);
    ...
    public JTextField addStringField(String label, String defaultValue, int columns) {
        ...
        row.add(tf);
        attachControlHelp(label, lbl, tf);
    ...
    public JComboBox<String> addChoice(String label, String[] items, String defaultItem) {
        ...
        row.add(combo);
        attachControlHelp(label, lbl, combo);
    ...
    /**
     * Compact sibling of {@link #addCardChoice}: icon-less, tighter cards so a
     * row of three or four options fits a narrow dialog. Registers the backing
     * combo in the choice sequence exactly like {@link #addCardChoice}.
     */
    public JComboBox<String> addCompactCardChoice(String header, CardChoice.Option[] options,
                                                  String defaultValue, CardChoice[] panelOut) {
        if (header != null && !header.isEmpty()) {
            addSubHeader(header);
        }
        CardChoice cards = new CardChoice(options, defaultValue, true);
        combos.add(cards.comboBox());
        if (panelOut != null && panelOut.length > 0) {
            panelOut[0] = cards;
        }
