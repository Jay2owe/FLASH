...
public final class ConfigQcContext {
    ...
    /** Default filtered-stack source-kind tag: raw (non-deconvolved) pixels. */
    private static final String SOURCE_KIND_RAW = "RAW";
    ...
    private final DeconvMirrorCache deconvMirrorCache;
    ...
    public ConfigQcContext(File projectDirectory, File binFolder, Object config,
        ...
        this(projectDirectory, binFolder, config, images, channelNames, channelIndex,
                filteredStackCache, null);
    }

    /**
     * Full constructor, adding the optional shared {@link DeconvMirrorCache} (Stage&nbsp;12). The cache
     * is nullable and backward-compatible: every existing constructor delegates here with {@code null},
     * so callers/tests that predate deconvolution keep compiling unchanged. When present, it maps each
     * {@code (seriesIndex, channelIndex)} to the full-Z deconvolution mirror computed during the QC pass
     * so downstream setup steps (Stage&nbsp;13) can preview the exact routed pixels.
     */
    public ConfigQcContext(File projectDirectory, File binFolder, Object config,
                           List<ConfigQcImage> images, List<String> channelNames,
                           int channelIndex, FilteredStackCache filteredStackCache,
                           DeconvMirrorCache deconvMirrorCache) {
        ...
        this.filteredStackCache = filteredStackCache == null
                ...
                : filteredStackCache;
        this.deconvMirrorCache = deconvMirrorCache;
    ...
    /**
     * The shared full-Z deconvolution-mirror cache populated by the Set&nbsp;Up&nbsp;Configuration QC pass
     * (Stage&nbsp;12), or {@code null} when this context was built without one (every non-deconv path).
     * Stage&nbsp;13 threads the same instance into the downstream stages' contexts so they preview the
     * routed pixels.
     */
    public DeconvMirrorCache getDeconvMirrorCache() {
        return deconvMirrorCache;
    }

    /**
     * The deconvolution-mirror record for one {@code (seriesIndex, channelIndex)}, or {@code null} when
     * no cache is attached or nothing was computed for that pair. A returned record with
     * {@link DeconvMirrorCache.Mirror#rawFallback} = {@code true} means the QC pass deliberately fell
     * back to raw (memory shortage / PSF failure / cancel) — distinct from a {@code null} "not computed".
     */
    public DeconvMirrorCache.Mirror getDeconvMirror(int seriesIndex, int channelIndex) {
        return deconvMirrorCache == null ? null : deconvMirrorCache.get(seriesIndex, channelIndex);
    }
    ...
    public void cacheCurrentFilteredStack(String macroContent, ImagePlus filteredStack) {
        cacheCurrentFilteredStack(macroContent, SOURCE_KIND_RAW, "", filteredStack);
    }

    /**
     * Stage&nbsp;13 overload: cache a filtered stack keyed additionally by the routed pixel source
     * ({@code sourceKind} = {@code "RAW"}/{@code "DECONV"}) and, when deconvolved, the mirror's params
     * fingerprint. This prevents a RAW and a DECONV filtered stack for the same channel/macro from
     * colliding, and prevents a params change from serving a stale deconvolved filtered stack.
     */
    public void cacheCurrentFilteredStack(String macroContent, String sourceKind,
                                          String deconvFingerprint, ImagePlus filteredStack) {
        cacheFilteredStackForCurrentImage(channelIndex, macroContent, sourceKind, deconvFingerprint,
                filteredStack);
    }
    ...
    public synchronized void cacheFilteredStackForCurrentImage(int channelIndex,
        ...
        cacheFilteredStackForCurrentImage(channelIndex, macroContent, SOURCE_KIND_RAW, "", filteredStack);
    }

    public synchronized void cacheFilteredStackForCurrentImage(int channelIndex,
                                                              String macroContent,
                                                              String sourceKind,
                                                              String deconvFingerprint,
                                                              ImagePlus filteredStack) {
        if (filteredStack == null || !hasText(macroContent)) return;
        filteredStackCache.put(
                filteredStackKey(channelIndex, macroContent, sourceKind, deconvFingerprint),
                filteredStack);
    }

    public ImagePlus duplicateCurrentFilteredStack(String macroContent) {
        return duplicateCurrentFilteredStack(macroContent, SOURCE_KIND_RAW, "");
    }

    /** Stage&nbsp;13 overload; see {@link #cacheCurrentFilteredStack(String, String, String, ImagePlus)}. */
    public ImagePlus duplicateCurrentFilteredStack(String macroContent, String sourceKind,
                                                   String deconvFingerprint) {
        return duplicateFilteredStackForCurrentImage(channelIndex, macroContent, sourceKind,
                deconvFingerprint);
    }

    public synchronized ImagePlus duplicateFilteredStackForCurrentImage(int channelIndex,
                                                                       String macroContent) {
        return duplicateFilteredStackForCurrentImage(channelIndex, macroContent, SOURCE_KIND_RAW, "");
    }
    ...
    public synchronized ImagePlus duplicateFilteredStackForCurrentImage(int channelIndex,
        ...
        return filteredStackCache.duplicate(
                filteredStackKey(channelIndex, macroContent, sourceKind, deconvFingerprint));
    }
    ...
    /**
     * Remove every cached filtered stack (all series, all source kinds/params) for one channel. Used by
     * Stage&nbsp;13's param-change invalidation after the deconvolution QC pass repopulates a channel's
     * mirror, so downstream stages re-filter on the new pixels instead of a stack built on the old ones.
     */
    public synchronized void clearFilteredStackCacheForChannel(int channelIndex) {
        filteredStackCache.removeForChannel(channelIndex);
    }
    ...
    private FilteredStackKey filteredStackKey(int channelIndex, String macroContent,
                                              String sourceKind, String deconvFingerprint) {
        return new FilteredStackKey(currentSeriesIndex(), Math.max(0, channelIndex),
                macroFingerprint(macroContent), normalizeSourceKind(sourceKind), safe(deconvFingerprint));
    }

    private static String normalizeSourceKind(String sourceKind) {
        String value = safe(sourceKind).trim();
        return value.isEmpty() ? SOURCE_KIND_RAW : value;
    }
    ...
    public static final class FilteredStackCache {
        ...
        /**
         * Remove every cached filtered stack for {@code channelIndex} across all series and all source
         * kinds/params fingerprints. Stage&nbsp;13 calls this after a channel's deconvolution mirror is
         * (re)computed so stale filtered stacks built on the old pixels are dropped.
         */
        public synchronized void removeForChannel(int channelIndex) {
            int safeChannelIndex = Math.max(0, channelIndex);
            List<FilteredStackKey> keysToRemove = new ArrayList<FilteredStackKey>();
            for (FilteredStackKey key : stacks.keySet()) {
                if (key.channelIndex == safeChannelIndex) {
                    keysToRemove.add(key);
                }
            }
            for (FilteredStackKey key : keysToRemove) {
    ...
    private static final class FilteredStackKey {
        ...
        final String deconvFingerprint;
        ...
        FilteredStackKey(int seriesIndex, int channelIndex, String macroHash,
            ...
            this.deconvFingerprint = safe(deconvFingerprint);
        }
        ...
        @Override public boolean equals(Object obj) {
            ...
            return seriesIndex == other.seriesIndex
                    ...
                    && deconvFingerprint.equals(other.deconvFingerprint);
        }
        ...
        @Override public int hashCode() {
            ...
            result = 31 * result + deconvFingerprint.hashCode();
            return result;
        }
    }
}
