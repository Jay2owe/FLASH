...
public final class FilterParameterStage implements ConfigQcStage {
    ...
    boolean previewWorkerBackgroundDoneForTest() {
    ...
    private void runPreviewOnWorker() {
        ...
        final AtomicReference<ImagePlus> resultHolder = new AtomicReference<ImagePlus>();
        ...
        previewWorker = new SwingWorker<ImagePlus, Void>() {
            @Override protected ImagePlus doInBackground() throws Exception {
                ImagePlus result = previewAdapter.createFilteredPreview(previewSource, macro);
                resultHolder.set(result);
                if (isCancelled() || !isCurrentPreviewRequest(
                        this, requestEpoch, previewContext, previewSource)) {
                    closePreviewResult(resultHolder.getAndSet(null));
                }
                return result;
            }

            @Override protected void done() {
                boolean current = !isCancelled() && isCurrentPreviewRequest(
                        this, requestEpoch, previewContext, previewSource);
                ImagePlus result = resultHolder.getAndSet(null);
                try {
                    if (current) {
                        if (result == null) result = get();
                        installAdjustedPreview(result, macro);
                    } else {
                        closePreviewResult(result);
                    }
                } catch (Exception e) {
                    if (current) {
                    ...
                    if (previewWorker == this) previewWorker = null;
    ...
    private void openMacroVariationsDialog() {
        ...
        // never re-emitting through the DAG.
    ...
    private void closePreviewResult(ImagePlus image) {
        if (image != null) previewAdapter.close(image);
    }
