...
public final class FilterParameterStage implements ConfigQcStage {
    ...
    private void loadSavedState() {
        if (restartMacro != null) {
            ...
            installLoadedMacro(savedMacro);
            if (hasMacro(restartDisplayMacro)) {
                currentDisplayMacro = restartDisplayMacro;
            }
            baseMacro = currentMacro;
            dirty = false;
            readOnlyBase = isBundledPreset(selectedPreset);
            structurallyMutated = restartStructurallyMutated || structurallyMutated;
    ...
    private void installLoadedMacro(String macro) {
        currentMacro = safe(macro);
        ...
        DagIR embedded = null;
        try {
            embedded = IjmToDagLoader.loadEmbeddedDag(currentMacro);
        } catch (RuntimeException ignored) {
            embedded = null;
        }

        if (embedded != null && embedded.isLinear()) {
            try {
                hiddenBuilder = new FilterBuilderPanel(embedded, /*sharedPreview=*/null,
                        /*runner=*/null, null);
    ...
    private static String extractArgsForNodeIndex(String macro,
        ...
        int idx = 0;
        while (m.find()) {
            if (isEmitterBoilerplateCommand(m.group(1))) continue;
            if (idx == targetIndex) return m.group(2);
            idx++;
        }
        return null;
    }
    ...
    private void ensureHiddenBuilderInSyncWithCurrentMacro() {
        ...
        try {
            ...
            while (m.find() && idx < summaries.size()) {
                if (isEmitterBoilerplateCommand(m.group(1))) continue;
                hiddenBuilder.updateNodeArgs(summaries.get(idx).id, m.group(2));
                idx++;
            }
    ...
    private static boolean isEmitterBoilerplateCommand(String command) {
