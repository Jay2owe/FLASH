...
public final class IdentityInferenceDialog {
    ...
    private void open() {
        ...
        pd.addHelpText("Rule: only the fields the chosen rule uses matter; the rest are ignored.");
        pd.addHelpText("Token number splits the filename on underscores and takes the Nth piece.");
        pd.addHelpText("Capture regex: Java regex; the first capture group becomes the field value.");
    ...
    private JComponent buildForm() {
        ...
        form.add(tokenField);
        form.add(headed("Condition axis label (e.g. Genotype):"));
        axisField.setToolTipText("Type a new name (e.g. Genotype, Timepoint) to create a second condition "
                + "axis, or keep Condition. Multiple axes form a condition matrix.");
        form.add(axisField);
