...
public final class IdentityGrammarDialog {
    ...
    private void open() {
        ...
        validationStatus.setForeground(FlashTheme.TEXT_MUTED);
        validationStatus.getAccessibleContext().setAccessibleName("Grammar validation status");
        pd.addComponent(validationStatus);
    ...
    private void refreshPreview() {
        try {
            ...
            validationStatus.setForeground(FlashTheme.SUCCESS_FG);
            validationStatus.setText("Grammar patterns are valid and unambiguous for this preview.");
        } catch (RuntimeException ex) {
            preview.render(null);
            validationStatus.setForeground(FlashTheme.DANGER_FG);
            validationStatus.setText(validationMessage(ex));
    ...
    private void loadGrammar() {
        ...
        NamingGrammarStore.LoadResult result = NamingGrammarStore.loadResult(projectDir, chosen);
        if (result.isOk()) {
            populateFrom(result.grammar);
            refreshPreview();
            return;
        }
        JOptionPane.showMessageDialog(owner, loadDiagnostic(result),
                "Identity grammar - " + loadStateLabel(result.state),
                result.state == NamingGrammarStore.LoadState.ABSENT
    ...
    private static String validationMessage(RuntimeException error) {
        String detail = error == null || error.getMessage() == null
                ? "Unknown validation error." : error.getMessage();
        if (error instanceof flash.pipeline.intelligence.identity.ValuePattern.UnsafePatternException) {
            return "Unsafe pattern: " + detail;
        }
        if (error instanceof flash.pipeline.intelligence.identity.GrammarInterpreter.AmbiguousMatchException) {
            return "Ambiguous match: " + detail;
        }
        return "Grammar validation failed: " + detail;
    }

    private static String loadDiagnostic(NamingGrammarStore.LoadResult result) {
        String prefix;
        switch (result.state) {
            case ABSENT: prefix = "The selected grammar is no longer present."; break;
            case CORRUPT: prefix = "The saved grammar is corrupt and was not loaded."; break;
            case UNSUPPORTED: prefix = "The saved grammar was created by a newer FLASH version."; break;
            case UNSAFE: prefix = "The saved grammar contains an unsafe name or pattern."; break;
            default: prefix = "The saved grammar could not be loaded."; break;
        }
        return result.diagnostic.isEmpty() ? prefix : prefix + "\n\n" + result.diagnostic;
    }

    private static String loadStateLabel(NamingGrammarStore.LoadState state) {
        if (state == null) return "load failed";
        switch (state) {
            case ABSENT: return "not found";
            case CORRUPT: return "corrupt file";
            case UNSUPPORTED: return "newer version";
            case UNSAFE: return "unsafe pattern";
