...
public final class ObjectClassifierScorer {
    ...
    public ImagePlus filterLabelImage(ImagePlus labelImage, List<ScoreResult> results) {
        ...
        if (filtered != null) {
            // ImageOps deliberately copies arbitrary ImagePlus properties. The
            // StarDist table is keyed by the source labels, while this method
            // removes objects and assigns a new consecutive label domain. Do
            // not expose those stale rows to strict downstream pixel/table
            // consumers. RF score metadata is attached separately by the
            // TrainedRfRunner after relabeling.
            filtered.setProperty(StarDist3DRunner.OBJECT_STATS_PROPERTY, null);
        }
