...
public final class StarDistFilterSuggester
    ...
    public StarDistSuggestion suggest(SuggestionContext ctx) {
        ...
        Set<Integer> pixelLabels = canonicalLabels(ctx.labelImage);
        final Map<Integer, StatRow> rows = rowsByLabel(table);
        if (!pixelLabels.equals(rows.keySet())) {
            Set<Integer> missingRows = new TreeSet<Integer>(pixelLabels);
            missingRows.removeAll(rows.keySet());
            Set<Integer> unexpectedRows = new TreeSet<Integer>(rows.keySet());
            unexpectedRows.removeAll(pixelLabels);
            throw new IllegalArgumentException("LABEL_IDENTITY_UNSUPPORTED: StarDist suggestion "
                    + "pixel/statistics label sets differ; missing statistics=" + missingRows
                    + ", statistics without pixels=" + unexpectedRows + ".");
        }
        if (rows.isEmpty()) {
            return StarDistSuggestion.none();
        }
    ...
    private static Map<Integer, StatRow> rowsByLabel(ResultsTable table) {
        ...
        for (int row = 0; row < table.size(); row++) {
            ...
            StatRow previous = out.put(Integer.valueOf(label), new StatRow(
            ...
            if (previous != null) {
                throw new IllegalArgumentException("LABEL_IDENTITY_UNSUPPORTED: duplicate "
                        + "StarDist statistics label " + label + " at row " + row + ".");
            }
        }
        return out;
    }
    ...
    private static int requiredLabelForRow(ResultsTable table, int row) {
        try {
            ...
            throw new IllegalArgumentException("LABEL_IDENTITY_UNSUPPORTED: missing canonical "
                    + "StarDist statistics Label at row " + row + ".", e);
        }
    }

    private static Set<Integer> canonicalLabels(ij.ImagePlus labelImage) {
        ...
        for (int slice = 1; slice <= stack.getSize(); slice++) {
            ...
            for (int pixel = 0; pixel < processor.getPixelCount(); pixel++) {
                ...
                labels.add(Integer.valueOf(checkedPositiveLabel(value,
                        "StarDist suggestion pixel " + pixel + " in slice " + slice)));
            }
        }
        return labels;
    }
    ...
    private static int checkedPositiveLabel(double value, String location) {
        if (!Double.isFinite(value) || value <= 0.0d
            ...
            throw new IllegalArgumentException("LABEL_IDENTITY_UNSUPPORTED: " + location
                    + " must be a positive integral label no greater than "
