...
public class StarDist3DRunner {
    ...
    public static ImagePlus run(ImagePlus input, double probThresh, double nmsThresh,
        ...
        try {
            ...
            // TrackMate exports 32-bit float labels. Keep that representation when an
            // ID exceeds the unsigned-short range; unchecked convertToShort() clips
            // distinct objects to the same value. Small label domains retain the
            // historical 16-bit representation after an exact checked conversion.
            prepareLabelImageForInstall(labelImp, "StarDist");
            ...
            ResultsTable objectStats = statsProperty instanceof ResultsTable
                    ...
                    : new ResultsTable();
            validatePixelTableJoin(labelImp, objectStats);
    ...
    static ResultsTable buildObjectStats(Model model) {
        ...
        Set<Integer> mappedLabels = new HashSet<Integer>();
        int row = 0;
        for (Integer trackId : trackIds) {
            Set<Spot> spots = model.getTrackModel().trackSpots(trackId);
            if (spots == null || spots.isEmpty()) continue;
            int mappedLabel = labelImgExporterTrackLabel(trackId);
            if (!mappedLabels.add(Integer.valueOf(mappedLabel))) {
                throw new IllegalStateException("StarDist produced duplicate mapped label "
                        + mappedLabel + " for TrackMate track " + trackId + ".");
            }
            ...
            table.setValue("Label", row, mappedLabel);
    ...
    /**
     * Mirrors TrackMate's {@code LABEL_IS_TRACK_ID} contract: zero is reserved
     * for background, so exported pixels contain {@code trackId + 1}.
     */
    static int labelImgExporterTrackLabel(Integer trackId) {
        if (trackId == null || trackId.intValue() < 0
                || trackId.intValue() >= MAX_EXACT_FLOAT_LABEL) {
            throw new IllegalArgumentException("TrackMate track ID must be between 0 and "
                    + (MAX_EXACT_FLOAT_LABEL - 1) + " for an exact ImageJ label pixel: "
                    + trackId);
        }
    ...
    /**
     * Validates the integral label domain before installing runner output and
     * narrows to 16-bit only when every value is exactly representable.
     */
    static void prepareLabelImageForInstall(ImagePlus labelImage, String producer) {
        Set<Integer> before = validatedPositiveLabels(labelImage, producer);
        int maximum = 0;
        ...
        if (maximum <= 65_535 && labelImage.getBitDepth() != 16) {
            ...
            for (int s = 1; s <= oldStack.getSize(); s++) {
                ...
                for (int i = 0; i < source.getPixelCount(); i++) {
                    target.set(i, checkedLabel(source.getf(i), producer));
                }
            ...
            throw new IllegalArgumentException(producer
                    + " label IDs above 65,535 require 32-bit ImageJ storage.");
        }
        Set<Integer> after = validatedPositiveLabels(labelImage, producer);
        if (!before.equals(after)) {
            throw new IllegalStateException(producer
                    + " label storage conversion did not preserve the label identity set.");
        }
    }

    static void validatePixelTableJoin(ImagePlus labelImage, ResultsTable objectStats) {
        Set<Integer> pixels = validatedPositiveLabels(labelImage, "StarDist");
        Set<Integer> rows = new HashSet<Integer>();
        if (objectStats != null) {
            for (int row = 0; row < objectStats.size(); row++) {
                int label = requiredTableLabel(objectStats, row);
                if (!rows.add(Integer.valueOf(label))) {
                    throw new IllegalArgumentException("StarDist object statistics contain duplicate label "
                            + label + ".");
                }
            }
        }
        if (!pixels.equals(rows)) {
            throw new IllegalStateException("StarDist pixel/table label mismatch: pixels="
                    + pixels + ", table=" + rows + ".");
        }
    }
    ...
    public static int applyObjectFilters(ImagePlus labelImage,
                                         ...
                                         double qualityMin,
                                         double intensityMin) {
        if (labelImage == null || objectStats == null) return 0;
        validatePixelTableJoin(labelImage, objectStats);
        if (objectStats.size() == 0) return 0;
        ...
        for (int row = 0; row < objectStats.size(); row++) {
            ...
            if (remove) {
                rowsToRemove.add(Integer.valueOf(row));
                if (label <= MAX_BITSET_LABEL) {
        ...
        for (int index = rowsToRemove.size() - 1; index >= 0; index--) {
            objectStats.deleteRow(rowsToRemove.get(index).intValue());
        }
        validatePixelTableJoin(labelImage, objectStats);
    ...
    private static int requiredTableLabel(ResultsTable table, int row) {
        try {
            double label = table.getValue("Label", row);
            if (!Double.isFinite(label) || label <= 0.0 || label != Math.rint(label)
                    || label > MAX_EXACT_FLOAT_LABEL) {
                throw new IllegalArgumentException("Invalid StarDist object-statistics label at row "
                        + row + ": " + label);
            }
            return (int) label;
        } catch (RuntimeException e) {
            if (e instanceof IllegalArgumentException) throw e;
            throw new IllegalArgumentException("Missing StarDist object-statistics Label at row "
                    + row + ".", e);
        }
    }

    private static Set<Integer> validatedPositiveLabels(ImagePlus labelImage, String producer) {
        if (labelImage == null || labelImage.getStack() == null) {
            throw new IllegalArgumentException(producer + " label image is missing.");
        }
        if (labelImage.getBitDepth() == 24) {
            throw new IllegalArgumentException(producer + " label image cannot use RGB storage.");
        }
        Set<Integer> labels = new HashSet<Integer>();
        for (int s = 1; s <= labelImage.getStackSize(); s++) {
            ImageProcessor processor = labelImage.getStack().getProcessor(s);
            if (processor == null) {
                throw new IllegalArgumentException(producer + " label image has a missing slice " + s + ".");
            }
            for (int i = 0; i < processor.getPixelCount(); i++) {
                int label = checkedLabel(processor.getf(i), producer);
    ...
    private static int checkedLabel(float value, String producer) {
        if (!Float.isFinite(value) || value < 0.0f || value != Math.rint(value)
                || value > MAX_EXACT_FLOAT_LABEL) {
            throw new IllegalArgumentException(producer
                    + " labels must be finite, integral, non-negative, and no greater than "
