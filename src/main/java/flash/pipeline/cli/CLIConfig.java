...
public class CLIConfig {
    ...
    // Stage 15 (deconv routing §C3): whether each downstream deconv toggle was EXPLICITLY set on
    // the CLI. Null means "flag absent" so the routing resolver can tell an explicit hard override
    // apart from the default; the primitives above keep today's default (true) for existing readers.
    Boolean splitMergeUseDeconvExplicit = null;
    Boolean threeDUseDeconvExplicit = null;
    Boolean intensityV2UseDeconvExplicit = null;
    ...
    /** The explicit CLI value for the 3D Object deconv toggle, or {@code null} if the flag was absent. */
    public Boolean getThreeDUseDeconvExplicit() { return threeDUseDeconvExplicit; }
    /** The explicit CLI value for the Intensity V2 deconv toggle, or {@code null} if the flag was absent. */
    public Boolean getIntensityV2UseDeconvExplicit() { return intensityV2UseDeconvExplicit; }
    /** The explicit CLI value for the Split/Merge (Display) deconv toggle, or {@code null} if absent. */
    public Boolean getSplitMergeUseDeconvExplicit() { return splitMergeUseDeconvExplicit; }
    ...
    public String toMacroOptions() {
        ...
        if (deconv.requireFresh) {
            parts.add("deconv.requireFresh=true");
        }
        ...
        if (object.doBBCpc != null) {
            parts.add("object.doBBCpc=" + object.doBBCpc);
        }
        if (object.doBBVol != null) {
            parts.add("object.doBBVol=" + object.doBBVol);
        }
        if (object.doRadialProfile != null) {
            parts.add("object.oip.radial=" + object.doRadialProfile);
        }
        if (object.doMarginalProfile != null) {
            parts.add("object.oip.marginal=" + object.doMarginalProfile);
        }
        if (object.doPrincipalAxisProfile != null) {
            parts.add("object.oip.principal_axis=" + object.doPrincipalAxisProfile);
        }
        if (object.doAngularProfile != null) {
            parts.add("object.oip.angular=" + object.doAngularProfile);
        }
        if (object.doShellColoc != null) {
            parts.add("object.oip.shell=" + object.doShellColoc);
        }
        if (object.doWithinBoxCorr != null) {
            parts.add("object.oip.within_box=" + object.doWithinBoxCorr);
        }
        if (object.oipGenerateFigures != null) {
            parts.add("object.oip.figures=" + object.oipGenerateFigures);
        }
        if (object.oipRegion != null && !object.oipRegion.trim().isEmpty()) {
            parts.add("object.oip.region=" + object.oipRegion.trim());
        }
        if (object.oipIntensityNorm != null && !object.oipIntensityNorm.trim().isEmpty()) {
            parts.add("object.oip.norm=" + object.oipIntensityNorm.trim());
        }
        if (object.oipRadialBins != null) {
            parts.add("object.oip.radial_bins=" + object.oipRadialBins);
        }
        if (object.oipAngularBins != null) {
            parts.add("object.oip.angular_bins=" + object.oipAngularBins);
        }
        if (object.oipShells != null) {
            parts.add("object.oip.shells=" + object.oipShells);
        }
        if (object.oipResampleN != null) {
            parts.add("object.oip.resample_n=" + object.oipResampleN);
        }
        if (object.oipBoxPadPct != null) {
            parts.add("object.oip.box_pad_pct=" + formatDouble(object.oipBoxPadPct.doubleValue()));
        }
        if (object.oipRingThresholdPct != null) {
            parts.add("object.oip.ring_threshold_pct="
                    + formatDouble(object.oipRingThresholdPct.doubleValue()));
        }
        ...
        // The shared encoder writes an explicitly-empty set as `key=[]`, preserving
        // the distinction between a present empty selection and an omitted key.
        ...
        if (aggregate.presetName != null && !aggregate.presetName.trim().isEmpty()) {
            ...
            parts.add("aggregate.preset=" + trimmed);
        }
        ...
        if (excel.presetName != null && !excel.presetName.trim().isEmpty()) {
            ...
            parts.add("excel.preset=" + trimmed);
        }
        ...
        if (stats.presetName != null && !stats.presetName.trim().isEmpty()) {
            ...
            parts.add("stats.preset=" + trimmed);
        }
        ...
        if (stats.metrics != null && !stats.metrics.isEmpty()) {
            ...
            parts.add("stats.metrics=" + joined);
        }
        if (stats.metricAggregations != null && !stats.metricAggregations.isEmpty()) {
            ...
            if (!joined.isEmpty()) {
                parts.add("stats.metric_aggregation=" + joined);
            }
        }
        ...
        for (String part : parts) {
            ...
            if (separator < 0) {
                ...
                sb.append(part, 0, separator + 1)
                        .append(CLIArgumentParser.encodeValue(part.substring(separator + 1)));
            }
        }
        return sb.toString();
    }
    ...
    public static final class DeconvConfig {
        ...
        // Stage 17 preflight strictness: when true, a headless/CLI consumer that finds a missing or
        // stale deconvolved mirror for its routed channels hard-fails the run instead of warning and
        // proceeding on raw. Independent of deconv.enabled (a strict pipeline expects run_deconv to
        // have produced fresh mirrors first). Default false = warn + proceed on raw.
        boolean requireFresh = false;
        final Map<Integer, ChannelOverride> perChannel = new LinkedHashMap<Integer, ChannelOverride>();
        ...
        /** When true, a headless consumer hard-fails on a missing/stale routed deconvolution mirror. */
        public boolean isRequireFresh() { return requireFresh; }
        public Map<Integer, ChannelOverride> getPerChannel() { return perChannel; }

        /** Per-channel deconvolution overrides parsed from {@code deconv.ch<N>.*} flags. */
    ...
    public static final class ThreeDObjectConfig {
        ...
        Boolean doRadialProfile = null;
        Boolean doMarginalProfile = null;
        Boolean doPrincipalAxisProfile = null;
        Boolean doAngularProfile = null;
        Boolean doShellColoc = null;
        Boolean doWithinBoxCorr = null;
        Boolean oipGenerateFigures = null;
        String oipRegion = null;
        String oipIntensityNorm = null;
        Integer oipRadialBins = null;
        Integer oipAngularBins = null;
        Integer oipShells = null;
        Integer oipResampleN = null;
        Double oipBoxPadPct = null;
        Double oipRingThresholdPct = null;
        ...
        public Boolean getDoRadialProfile() { return doRadialProfile; }
        public Boolean getDoMarginalProfile() { return doMarginalProfile; }
        public Boolean getDoPrincipalAxisProfile() { return doPrincipalAxisProfile; }
        public Boolean getDoAngularProfile() { return doAngularProfile; }
        public Boolean getDoShellColoc() { return doShellColoc; }
        public Boolean getDoWithinBoxCorr() { return doWithinBoxCorr; }
        public Boolean getOipGenerateFigures() { return oipGenerateFigures; }
        public String getOipRegion() { return oipRegion; }
        public String getOipIntensityNorm() { return oipIntensityNorm; }
        public Integer getOipRadialBins() { return oipRadialBins; }
        public Integer getOipAngularBins() { return oipAngularBins; }
        public Integer getOipShells() { return oipShells; }
        public Integer getOipResampleN() { return oipResampleN; }
        ...
        public boolean hasConfiguration() {
            return hasSetupConfiguration() || hasOipConfiguration();
        }

        public boolean hasSetupConfiguration() {
            return (presetName != null && !presetName.trim().isEmpty())
        ...
        public boolean hasOipConfiguration() {
            return doRadialProfile != null
                    ...
                    || (oipRegion != null && !oipRegion.trim().isEmpty())
                    || (oipIntensityNorm != null && !oipIntensityNorm.trim().isEmpty())
                    || oipRadialBins != null
                    || oipAngularBins != null
                    || oipShells != null
                    || oipResampleN != null
    ...
    public static final class RepfigConfig {
        ...
        void appendMacroOptions(List<String> parts) {
            ...
            if (saveName != null && !saveName.trim().isEmpty()) {
                ...
                parts.add("repfig.save_name=" + clean);
            }
        }
