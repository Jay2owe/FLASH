...
public final class RegionMask {
    ...
    /** Largest integer exactly representable by ImageJ's 32-bit float labels. */
    ...
    public static int filterLabelsByCentroid(ImagePlus labelImage, Roi roi) {
        ...
        Map<Integer, long[]> centroids = new LinkedHashMap<Integer, long[]>();
        for (int s = 1; s <= nSlices; s++) {
            ImageProcessor ip = stack.getProcessor(s);
            int nPixels = ip.getPixelCount();
            int w = ip.getWidth();
            for (int i = 0; i < nPixels; i++) {
                int label = checkedLabel(ip.getf(i), s, i);
                if (label == 0) continue;
        ...
        ResultsTable starDistStats = starDistStats(labelImage);
        if (starDistStats != null) {
            validatePixelTableJoin(centroids.keySet(), starDistStats);
        }

        // Determine which labels have centroids outside the ROI
        Set<Integer> reject = new HashSet<Integer>();
        for (Map.Entry<Integer, long[]> entry : centroids.entrySet()) {
        ...
        ResultsTable retainedStats = starDistStats == null
                ? null
                : retainedStatsRows(starDistStats, reject);
        ...
        for (int s = 1; s <= nSlices; s++) {
            ...
            int nPixels = ip.getPixelCount();
            for (int i = 0; i < nPixels; i++) {
                if (reject.contains(Integer.valueOf(checkedLabel(ip.getf(i), s, i)))) {
                    ip.setf(i, 0.0f);
                }
            }
        }
        ...
        if (retainedStats != null) {
            labelImage.setProperty(StarDist3DRunner.OBJECT_STATS_PROPERTY, retainedStats);
            validatePixelTableJoin(validatedPixelLabels(labelImage), retainedStats);
        }

        return reject.size();
    }

    private static ResultsTable starDistStats(ImagePlus labelImage) {
        Object property = labelImage.getProperty(StarDist3DRunner.OBJECT_STATS_PROPERTY);
        return property instanceof ResultsTable ? (ResultsTable) property : null;
    }

    private static ResultsTable retainedStatsRows(ResultsTable source, Set<Integer> rejected) {
        ResultsTable retained = new ResultsTable();
        String[] headings = source.getHeadings();
        int targetRow = 0;
        for (int sourceRow = 0; sourceRow < source.size(); sourceRow++) {
            int label = requiredStatsLabel(source, sourceRow);
            if (rejected.contains(Integer.valueOf(label))) continue;
            retained.incrementCounter();
            if (headings != null) {
                for (String heading : headings) {
                    if (heading == null || heading.isEmpty()) continue;
                    try {
                        retained.setValue(heading, targetRow,
                                source.getValue(heading, sourceRow));
                    } catch (RuntimeException e) {
                        throw new IllegalArgumentException(
                                "Could not preserve StarDist statistics column '" + heading
                                        + "' at row " + sourceRow + ".", e);
                    }
                }
            }
    ...
    private static Set<Integer> validatedPixelLabels(ImagePlus labelImage) {
        Set<Integer> labels = new HashSet<Integer>();
        ImageStack stack = labelImage.getStack();
        for (int slice = 1; slice <= stack.getSize(); slice++) {
            ImageProcessor processor = stack.getProcessor(slice);
            for (int pixel = 0; pixel < processor.getPixelCount(); pixel++) {
                int label = checkedLabel(processor.getf(pixel), slice, pixel);
    ...
    private static void validatePixelTableJoin(Set<Integer> pixelLabels, ResultsTable table) {
        Set<Integer> tableLabels = new HashSet<Integer>();
        for (int row = 0; row < table.size(); row++) {
            int label = requiredStatsLabel(table, row);
            if (!tableLabels.add(Integer.valueOf(label))) {
                throw new IllegalArgumentException(
                        "StarDist object statistics contain duplicate label " + label + ".");
            }
        }
        if (!pixelLabels.equals(tableLabels)) {
            throw new IllegalStateException("StarDist pixel/table label mismatch after ROI filtering: pixels="
                    + pixelLabels + ", table=" + tableLabels + ".");
        }
    }

    private static int requiredStatsLabel(ResultsTable table, int row) {
        double label;
        try {
            label = table.getValue("Label", row);
        } catch (RuntimeException e) {
            throw new IllegalArgumentException(
                    "Missing StarDist object-statistics Label at row " + row + ".", e);
        }
        if (!Double.isFinite(label) || label <= 0.0d || label != Math.rint(label)
                || label > MAX_EXACT_FLOAT_LABEL) {
            throw new IllegalArgumentException(
                    "Invalid StarDist object-statistics label at row " + row + ": " + label);
        }
        return (int) label;
    }

    private static int checkedLabel(float value, int slice, int pixel) {
        if (!Float.isFinite(value) || value < 0.0f || value != Math.rint(value)
                || value > MAX_EXACT_FLOAT_LABEL) {
            throw new IllegalArgumentException("ROI label-image pixel " + pixel + " in slice "
                    + slice + " must be finite, integral, non-negative, and no greater than "
