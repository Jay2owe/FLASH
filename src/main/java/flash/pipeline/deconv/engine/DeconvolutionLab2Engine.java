 ...
 * command flags {@code -monitor 0 -verbose mute}. The algorithm runs on the calling thread via
 ...
 * <ul>
 *   <li><b>{@code -monitor 0}</b> - leaving it out attaches DeconvolutionLab2's default monitors,
 *   including a {@code TableMonitor}; {@code deconvolve(...)} then pops that monitor's panel up as a
 *   "Monitor of run" dialog. DeconvolutionLab2 treats the literal {@code no} token as a console
 *   monitor, so it still emits misleading status lines for in-memory runs. (DeconvolutionLab2
 *   quirk: in {@code Command.decodeMonitors} the literal {@code no} does not clear monitors, it adds a
 *   {@code ConsoleMonitor} — hence the harmless {@code Image:}/{@code PSF:}/"Impossible to load the
 *   reference image" console lines. The {@code 0} token fully disables monitors.)</li>
...
public final class DeconvolutionLab2Engine implements DeconvolutionEngine {
    ...
    @Override
    public boolean usesIterations(Algorithm algorithm) {
        return algorithm == Algorithm.RL
                || algorithm == Algorithm.RL_TV
                || algorithm == Algorithm.LANDWEBER;
    }

    @Override
    public boolean usesRegularization(Algorithm algorithm) {
        return algorithm == Algorithm.RL_TV
                || algorithm == Algorithm.TIKHONOV
                || algorithm == Algorithm.WIENER;
    }
    ...
    public ImagePlus deconvolve(ImagePlus stack, ImagePlus psf, DeconvParams params)
        ...
        String runDescription = describeRun(stack, psf, params);
        ...
        try {
            ...
            throw new DeconvolutionException(
                    "DeconvolutionLab2 deconvolution failed (" + runDescription + ").", t);
        }

        if (result == null) {
            throw new DeconvolutionException(
                    "DeconvolutionLab2 did not produce an output image (" + runDescription + ").");
        }
     ...
     * <ul>
     *   <li>{@code -monitor 0}: keeps DeconvolutionLab2 from opening its "Monitor of run" table
     *   dialog and suppresses the optional-reference-image console monitor output.</li>
    ...
    static String buildApiCommand(DeconvParams params) {
        return "-algorithm " + buildAlgorithmSpec(params) + " -monitor 0 -verbose mute";
    }
    ...
    private static String describeRun(ImagePlus stack, ImagePlus psf, DeconvParams params) {
        return "algorithm=" + (params == null || params.getAlgorithm() == null
                ? "unknown" : params.getAlgorithm().displayName())
                + ", iterations=" + (params == null ? "unknown" : String.valueOf(params.getIterations()))
                + ", regularization=" + (params == null ? "unknown" : formatDouble(params.getRegularization()))
                + ", image=" + sizeXYZ(stack)
                + ", psf=" + sizeXYZ(psf);
    }
    ...
    private static String sizeXYZ(ImagePlus image) {
        ...
        return image.getWidth() + "x" + image.getHeight() + "x" + image.getStackSize();
    }
