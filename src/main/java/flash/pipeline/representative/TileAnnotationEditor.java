...
public final class TileAnnotationEditor extends JDialog {
    ...
    private JPanel buildSidePanel() {
        ...
        side.add(sideHint("Colour of the scale bar and labels on the tile."));
        side.add(Box.createVerticalStrut(8));
        side.add(sectionLabel("Scale bar"));
        side.add(labelledRow("Length um", barUmField));
        side.add(sideHint("Bar length in microns; 0 hides the scale bar."));
    ...
    private static JLabel sideHint(String text) {
        JLabel hint = new JLabel(text);
        hint.setFont(hint.getFont().deriveFont(java.awt.Font.PLAIN, 11f));
        hint.setForeground(new java.awt.Color(117, 117, 117));
