...
public class RepresentativeFigureConfig {
    ...
    private final Map<String, SavedSeriesIdentity> savedSeriesIdentitiesByCondition =
            new LinkedHashMap<String, SavedSeriesIdentity>();
    ...
    public void clearFigureSpecificState() {
        selection = null;
        savedSeriesIdentitiesByCondition.clear();
        layout = null;
        tileConfig = null;
    ...
    public Map<String, Object> toMap() {
        ...
        root.put("schemaVersion", Integer.valueOf(serializationSchemaVersion()));
        if (hasSaveName()) {
        ...
        if (selection != null) {
            ...
            root.put("lockedSeries", selectionToList(
                    selection, savedSeriesIdentitiesByCondition));
        }
    ...
    private int serializationSchemaVersion() {
        if (selection == null) {
            return SCHEMA_VERSION;
        }
        for (String condition : selection.conditionNames()) {
            SavedSeriesIdentity identity = savedSeriesIdentitiesByCondition.get(condition);
    ...
    public void copyFrom(RepresentativeFigureConfig other) {
        ...
        savedSeriesIdentitiesByCondition.clear();
        savedSeriesIdentitiesByCondition.putAll(other.savedSeriesIdentitiesByCondition);
        layout = other.layout;
    ...
    public static RepresentativeFigureConfig fromMap(Map<String, Object> values) {
        ...
        int schemaVersion = savedSchemaVersion(values);
        ...
        config.selection = selectionFrom(
                values, config.savedSeriesIdentitiesByCondition, schemaVersion);
    ...
    private static Map<String, Object> existingResultToMap(
        ...
        out.put("externalImport", Boolean.valueOf(existing.externalImport));
        return out;
    }
    ...
    private static RepresentativeStatLoader.ExistingResultOption existingResultFromMap(
        ...
        return new RepresentativeStatLoader.ExistingResultOption(
                file, column, relative, booleanValue(map.get("externalImport"), false));
    }

    /**
     * Captures portable, index-independent identities for the current selection.
     * Call this only after the selection has been reconciled to current project metadata.
     */
    public void captureSelectionIdentities(String directory) {
        LinkedHashMap<String, SavedSeriesIdentity> captured =
                new LinkedHashMap<String, SavedSeriesIdentity>();
        if (selection != null) {
            for (Map.Entry<String, RepresentativeSeries> entry : selection.asMap().entrySet()) {
                ...
                captured.put(entry.getKey(), SavedSeriesIdentity.current(
                        directory, series.sourcePath(), series.seriesName()));
            }
        }
        savedSeriesIdentitiesByCondition.clear();
        savedSeriesIdentitiesByCondition.putAll(captured);
    }

    SavedSeriesIdentity savedIdentityForCondition(String condition) {
        return savedSeriesIdentitiesByCondition.get(
                RepresentativeSelection.conditionLabel(condition));
    }

    static String sourceKey(String directory, File source) {
        if (source == null) {
            return "";
        }
        File absoluteSource = canonicalOrAbsolute(source);
        File projectRoot = directory == null || directory.trim().isEmpty()
                ? null : canonicalOrAbsolute(new File(directory));
        if (projectRoot != null) {
            Path rootPath = projectRoot.toPath().normalize();
            Path sourcePath = absoluteSource.toPath().normalize();
            if (sourcePath.startsWith(rootPath)) {
                String relative = rootPath.relativize(sourcePath).toString()
                        .replace(File.separatorChar, '/');
                return "project:" + normalizePathIdentity(relative);
            }
        }
        return "external:" + normalizePathIdentity(
                absoluteSource.getAbsolutePath().replace(File.separatorChar, '/'));
    }

    static String imageKey(String seriesName) {
        return normalizeIdentity(seriesName);
    }
    ...
    private static String normalizePathIdentity(String value) {
    ...
    static String normalizeIdentity(String value) {
        String clean = string(value);
        return clean.isEmpty() ? "" : Normalizer.normalize(clean, Normalizer.Form.NFC);
    }

    private static List<Object> selectionToList(
            RepresentativeSelection selection,
            Map<String, SavedSeriesIdentity> identities) {
        ...
        for (Map.Entry<String, RepresentativeSeries> entry : selection.asMap().entrySet()) {
            ...
            SavedSeriesIdentity identity = identities == null
                    ? null : identities.get(entry.getKey());
            if (identity != null && identity.isComplete()) {
                row.put("identityVersion", Integer.valueOf(2));
                row.put("sourceKey", identity.sourceKey);
                row.put("imageKey", identity.imageKey);
            }
    ...
    private static RepresentativeSelection selectionFrom(
        ...
        for (Object rowValue : rows) {
            ...
            if (row.isEmpty()) {
                if (schemaVersion == SCHEMA_VERSION) {
                    throw new IllegalArgumentException(
                            "Representative schema 2 locked rows must be objects with complete identity.");
                }
                continue;
            }
            SavedSeriesIdentity identity = savedIdentityFromRow(row, schemaVersion);
            ...
            if (series != null) {
                ...
                if (identity != null) {
                    identities.put(condition, identity);
                }
            }
        }
    ...
    private static int savedSchemaVersion(Map<String, Object> values) {
        if (values == null || !values.containsKey("schemaVersion")
                || string(values.get("schemaVersion")).isEmpty()) {
            return 1;
        }
        int version = exactSchemaInteger(values.get("schemaVersion"), "schemaVersion");
        if (version != 1 && version != SCHEMA_VERSION) {
            throw new IllegalArgumentException(
                    "Unsupported representative figure schemaVersion " + version + ".");
        }
        return version;
    }

    private static SavedSeriesIdentity savedIdentityFromRow(
            Map<String, Object> row, int schemaVersion) {
        boolean hasIdentityVersion = row.containsKey("identityVersion");
        boolean hasSourceKey = row.containsKey("sourceKey");
        boolean hasImageKey = row.containsKey("imageKey");
        if (schemaVersion == 1) {
            if (hasIdentityVersion || hasSourceKey || hasImageKey) {
                throw new IllegalArgumentException(
                        "Legacy representative schema 1 cannot contain version-2 identity fields.");
            }
            return null;
        }
        ...
        if (!hasIdentityVersion) {
            throw new IllegalArgumentException(
                    "Representative schema 2 locked rows require identityVersion 2.");
        }
        int identityVersion = exactSchemaInteger(
                row.get("identityVersion"), "locked row identityVersion");
        if (identityVersion != 2) {
            throw new IllegalArgumentException(
                    "Unsupported representative locked row identityVersion "
                            + identityVersion + ".");
        }
        SavedSeriesIdentity identity = SavedSeriesIdentity.fromMap(row);
        if (identity == null || !identity.isComplete()) {
            throw new IllegalArgumentException(
                    "Representative schema 2 locked rows require complete sourceKey and imageKey values.");
        }
        validateV2CoreSeriesFields(row, identity);
        return identity;
    }

    private static void validateV2CoreSeriesFields(
            Map<String, Object> row, SavedSeriesIdentity identity) {
        String condition = normalizeIdentity(string(row.get("condition")));
        String seriesName = normalizeIdentity(string(row.get("seriesName")));
        String animal = normalizeIdentity(string(row.get("animal")));
        if (condition.isEmpty() || seriesName.isEmpty() || animal.isEmpty()) {
            throw new IllegalArgumentException(
                    "Representative schema 2 locked rows require condition, seriesName, and animal values.");
        }
        if (!seriesName.equals(identity.imageKey)) {
            throw new IllegalArgumentException(
                    "Representative schema 2 seriesName must match its imageKey.");
        }
        int seriesIndex = exactSchemaInteger(row.get("seriesIndex"), "locked row seriesIndex");
        int seriesNumber = exactSchemaInteger(row.get("seriesNumber"), "locked row seriesNumber");
        if (seriesIndex < 0 || seriesNumber <= 0 || seriesNumber != seriesIndex + 1) {
            throw new IllegalArgumentException(
                    "Representative schema 2 requires a nonnegative seriesIndex and matching one-based seriesNumber.");
        }
    }

    private static int exactSchemaInteger(Object value, String field) {
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "Representative " + field + " must be an exact integer.", e);
        }
    }
    ...
    static final class SavedSeriesIdentity {
        final String sourceKey;
        final String imageKey;

        SavedSeriesIdentity(String sourceKey, String imageKey) {
            this.sourceKey = normalizeIdentity(sourceKey);
            this.imageKey = normalizeIdentity(imageKey);
        }

        static SavedSeriesIdentity current(String directory,
                                           File source,
                                           String seriesName) {
            return new SavedSeriesIdentity(
                    sourceKey(directory, source), imageKey(seriesName));
        }

        static SavedSeriesIdentity fromMap(Map<String, Object> row) {
            if (row == null || row.isEmpty()) {
                return null;
            }
            String source = string(row.get("sourceKey"));
            String image = string(row.get("imageKey"));
            if (source.isEmpty() && image.isEmpty()) {
                return null;
            }
            return new SavedSeriesIdentity(source, image);
        }
