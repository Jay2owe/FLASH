...
public final class ImageSourceDispatcher {
    ...
    private static ProjectSources tryReadProjectSources(File selectedDirectory) {
        ...
        for (ProjectFile.Item item : project.items) {
            ...
            // Revalidate immediately before dispatch so runtime loading opens
            // only the resolver's real, confined path rather than a raw or
            // subsequently altered manifest string.
            File source = ProjectPathResolver.requireResolvedSource(item, outputRoot);
    ...
    public static DeferredImageSupplier createSupplier(String directory) throws Exception {
        ...
        if (mode == SourceMode.TIFF_LOOSE
            ...
            mode = modeAfterLooseTiffRelocationChoice(directory, mode, choice);
        }
        switch (mode) {
            case CONTAINER:
    ...
    static SourceMode modeAfterLooseTiffRelocationChoice(String directory,
                                                         SourceMode currentMode,
                                                         LooseTiffRelocator.Choice choice) {
        if (choice == LooseTiffRelocator.Choice.MOVED) {
            return detectMode(directory);
        }
        if (choice == LooseTiffRelocator.Choice.CANCELLED) {
            throw new IllegalArgumentException("Loose TIFF input setup was cancelled.");
        }
        return currentMode;
    }
