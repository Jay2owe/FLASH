...
public class CLIArgumentParserTest {
    ...
    public void usageNamesRunBinAsConfigurationSetup() {
        ...
        assertTrue(usage.contains("IJ.run(\"FLASH\""));
        assertTrue(usage.contains("ij.py.run_plugin(\"FLASH\""));
        assertTrue(usage.contains("ImageJ --headless --run \"FLASH\""));
        assertFalse(usage.contains("FLASH - The Pipeline for Fluorescence Automated Spatial Histology"));
    ...
    public void parse_windowsPathWithSpacesAndFlagLikeSubstring() {
        CLIConfig cfg = CLIArgumentParser.parse(
                "dir=[C:\\Data\\path with run_3d\\data] run_intensity");

        assertNotNull(cfg);
        assertEquals("C:\\Data\\path with run_3d\\data", cfg.getDirectory());
