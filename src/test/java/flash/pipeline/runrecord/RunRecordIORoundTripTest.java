...
public class RunRecordIORoundTripTest {
    ...
    @Test
    public void zeroAndTruncatedPublicationFaultsKeepLastGoodSnapshotReadable() throws Exception {
        File runsDir = temp.newFolder("faulted-runs");
        RunRecord record = record("LAST_GOOD", "SpatialAnalysis", 1_716_912_000_000L);
        record.status = "ok";
        File file = RunRecordIO.runFile(runsDir, record);
        RunRecordIO.writeSnapshot(file, record);

        RunRecord replacement = record("REPLACEMENT", "SpatialAnalysis", 1_716_912_001_000L);
        replacement.status = "failed";
        byte[] candidate = (RunRecordCodec.encode(replacement) + "\n")
                .getBytes(StandardCharsets.UTF_8);

        for (FileOperationFaults.Trigger trigger : Arrays.asList(
                FileOperationFaults.Trigger.ZERO_OUTPUT,
                FileOperationFaults.Trigger.TRUNCATED_OUTPUT)) {
            FileOperationFaults.FailureObservation observation =
                    FileOperationFaults.failSinglePublication(file, candidate, trigger);

            assertEquals(trigger, observation.getTrigger());
            assertEquals(file.toPath().toAbsolutePath().normalize(),
                    observation.getAffectedPath());
            assertTrue(observation.getFailureMessage().contains(trigger.getLabel()));
            assertTrue(observation.getFailureMessage().contains(file.getAbsolutePath()));
            assertArrayEquals("A rejected run-record artifact must leave the sentinel unchanged",
                    observation.getPriorGeneration(0),
                    observation.getCurrentGeneration(0));
            assertArrayEquals(observation.getPriorGeneration(0),
                    Files.readAllBytes(file.toPath()));
            assertTrue(observation.isStagingClean());

            RunRecord latest = RunRecordIO.readLatest(file);
            assertEquals("LAST_GOOD", latest.runId);
