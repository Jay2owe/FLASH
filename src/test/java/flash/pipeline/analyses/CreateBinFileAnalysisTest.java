...
public class CreateBinFileAnalysisTest {
    ...
    @Test
    public void fullResetPublishesFreshMetadataSizedGeneration() throws Exception {
        File projectRoot = temp.newFolder("full-reset-complete");
        File settingsDir = FlashProjectLayout.forDirectory(
                projectRoot.getAbsolutePath()).configurationWriteDir();
        assertTrue(settingsDir.mkdirs());
        ChannelConfigIO.write(settingsDir,
                ChannelConfigIO.fromBinUserConfig(twoChannelConfig()));
        FullResetAnalysis analysis = new FullResetAnalysis(FullResetAnalysis.Mode.COMPLETE);

        analysis.handleFullReset(projectRoot.getAbsolutePath(), settingsDir);

        ChannelConfig reset = ChannelConfigIO.read(settingsDir);
        assertTrue(analysis.receivedNullExisting);
        assertTrue(reset != null && ChannelConfigIO.isComplete(reset));
        assertEquals(3, reset.channels.size());
        assertEquals("Fresh DAPI", reset.channels.get(0).name);
        assertEquals("Fresh IBA1", reset.channels.get(1).name);
        assertEquals("Fresh GFAP", reset.channels.get(2).name);
    }

    @Test
    public void cancelDuringFullResetRestoresPriorCompleteGeneration() throws Exception {
        File projectRoot = temp.newFolder("full-reset-cancel");
        File settingsDir = FlashProjectLayout.forDirectory(
                projectRoot.getAbsolutePath()).configurationWriteDir();
        assertTrue(settingsDir.mkdirs());
        ChannelConfig prior = ChannelConfigIO.fromBinUserConfig(twoChannelConfig());
        prior.channels.get(0).markerId = "prior-marker";
        ChannelConfigIO.write(settingsDir, prior);
        FullResetAnalysis analysis = new FullResetAnalysis(FullResetAnalysis.Mode.DISCARD);

        analysis.handleFullReset(projectRoot.getAbsolutePath(), settingsDir);

        ChannelConfig restored = ChannelConfigIO.read(settingsDir);
        assertTrue(restored != null && ChannelConfigIO.isComplete(restored));
        assertEquals(2, restored.channels.size());
        assertEquals("prior-marker", restored.channels.get(0).markerId);
    }

    @Test
    public void saveAndExitDuringFullResetKeepsNewDraftAndRecoverablePriorGeneration() throws Exception {
        File projectRoot = temp.newFolder("full-reset-save-draft");
        File settingsDir = FlashProjectLayout.forDirectory(
                projectRoot.getAbsolutePath()).configurationWriteDir();
        assertTrue(settingsDir.mkdirs());
        ChannelConfigIO.write(settingsDir,
                ChannelConfigIO.fromBinUserConfig(twoChannelConfig()));
        FullResetAnalysis analysis = new FullResetAnalysis(FullResetAnalysis.Mode.SAVE_DRAFT);

        analysis.handleFullReset(projectRoot.getAbsolutePath(), settingsDir);

        ChannelConfig draft = ChannelConfigIO.read(settingsDir);
        assertTrue(draft != null);
        assertFalse(ChannelConfigIO.isComplete(draft));
        assertEquals(3, draft.channels.size());
        File[] preserved = settingsDir.listFiles((dir, name) ->
                name.startsWith("channel_config.corrupt-") && name.endsWith(".json"));
        assertTrue(preserved != null && preserved.length >= 1);
    }

    @Test
    public void failedFullResetRestoresPriorCompleteGeneration() throws Exception {
        File projectRoot = temp.newFolder("full-reset-failure");
        File settingsDir = FlashProjectLayout.forDirectory(
                projectRoot.getAbsolutePath()).configurationWriteDir();
        assertTrue(settingsDir.mkdirs());
        ChannelConfigIO.write(settingsDir,
                ChannelConfigIO.fromBinUserConfig(twoChannelConfig()));
        FullResetAnalysis analysis = new FullResetAnalysis(FullResetAnalysis.Mode.FAIL);

        try {
            analysis.handleFullReset(projectRoot.getAbsolutePath(), settingsDir);
            throw new AssertionError("Expected reset failure");
    ...
    @Test
    public void deterministicQcSelectionAndReplayUseStableIdsAcrossReordering() {
        String idA = CreateBinFileAnalysis.qcStableSeriesId("input/A.tif", 0);
        String idB = CreateBinFileAnalysis.qcStableSeriesId("input/B.tif", 0);
        String idC = CreateBinFileAnalysis.qcStableSeriesId("input/C.tif", 0);
        List<CreateBinFileAnalysis.QcSeriesCandidate> first = Arrays.asList(
                new CreateBinFileAnalysis.QcSeriesCandidate(0, idA),
                new CreateBinFileAnalysis.QcSeriesCandidate(1, idB),
                new CreateBinFileAnalysis.QcSeriesCandidate(2, idC));
        List<CreateBinFileAnalysis.QcSeriesCandidate> reordered = Arrays.asList(
                new CreateBinFileAnalysis.QcSeriesCandidate(0, idC),
                new CreateBinFileAnalysis.QcSeriesCandidate(1, idA),
                new CreateBinFileAnalysis.QcSeriesCandidate(2, idB));

        CreateBinFileAnalysis.QcSelectionPlan selected =
                CreateBinFileAnalysis.deterministicQcSelection(first, 2, "");
        CreateBinFileAnalysis.QcSelectionPlan selectedAgain =
                CreateBinFileAnalysis.deterministicQcSelection(reordered, 2, "");

        assertEquals(selected.seed, selectedAgain.seed);
        assertEquals(selected.stableIds, selectedAgain.stableIds);
        assertEquals(Arrays.asList(Integer.valueOf(
                        selected.stableIds.get(0).equals(idC) ? 0
                                : selected.stableIds.get(0).equals(idA) ? 1 : 2),
                        Integer.valueOf(selected.stableIds.get(1).equals(idC) ? 0
                                : selected.stableIds.get(1).equals(idA) ? 1 : 2)),
                CreateBinFileAnalysis.replayQcSelection(reordered, selected.stableIds));
    }

    @Test
    public void qcStableSeriesIdsPreserveCaseAndWhitespaceExactly() {
        String upper = CreateBinFileAnalysis.qcStableSeriesId(" input/SlideA.tif ", 0);
        String lower = CreateBinFileAnalysis.qcStableSeriesId(" input/slidea.tif ", 0);
        String trimmed = CreateBinFileAnalysis.qcStableSeriesId("input/SlideA.tif", 0);

        assertFalse(upper.equals(lower));
        assertFalse(upper.equals(trimmed));
        assertEquals(CreateBinFileAnalysis.qcStableSeriesId("input\\SlideA.tif", 0),
                CreateBinFileAnalysis.qcStableSeriesId("input/SlideA.tif", 0));
    }

    @Test
    public void qcSelectionProvenanceSurvivesChannelConfigSaveAndLoad() throws Exception {
        File project = temp.newFolder("qc-selection-provenance");
        File settings = configurationDir(project);
        CreateBinFileAnalysis.BinUserConfig cfg = twoChannelConfig();
        cfg.qcSelectedSeriesIds.add(CreateBinFileAnalysis.qcStableSeriesId("input/A.tif", 0));
        cfg.qcSelectedSeriesIds.add(CreateBinFileAnalysis.qcStableSeriesId("input/C.tif", 0));
        cfg.qcSelectionMode = "Randomly select images";
        cfg.qcSelectionAlgorithm = "sha256-ranked-v1";
        cfg.qcSelectionSeed = "recorded-seed";

        new CreateBinFileAnalysis().writeBinConfigFiles(settings, cfg);

        ChannelConfig raw = ChannelConfigIO.read(settings);
        assertEquals(cfg.qcSelectedSeriesIds, raw.extras.get("qcSelectedSeriesIds"));
        assertEquals("sha256-ranked-v1", raw.extras.get("qcSelectionAlgorithm"));
        CreateBinFileAnalysis.BinUserConfig replay =
                CreateBinFileAnalysis.importBinUserConfigFromSettingsDir(settings);
        assertEquals(cfg.qcSelectedSeriesIds, replay.qcSelectedSeriesIds);
        assertEquals(cfg.qcSelectionMode, replay.qcSelectionMode);
        assertEquals(cfg.qcSelectionAlgorithm, replay.qcSelectionAlgorithm);
        assertEquals(cfg.qcSelectionSeed, replay.qcSelectionSeed);
    }
    ...
    public void executeFiltered_visitsOnlyRequestedPagesAndPreservesExistingConfig() throws Exception {
        ...
        ChannelConfig rawUpdated = ChannelConfigIO.read(configurationDir(dir));
        assertEquals("neurons_neun", rawUpdated.channels.get(0).markerId);
        assertEquals("nuclei_dapi", rawUpdated.channels.get(1).markerId);
    ...
    @Test
    public void productionIdentityExtractionBindsDapiIba1AndLeavesFreeFormUnbound() throws Exception {
        CreateBinFileAnalysis analysis = new CreateBinFileAnalysis();
        CreateBinFileAnalysis.BinUserConfig draft = threeChannelConfig();
        CreateBinFileAnalysis.ChannelIdentityGrid grid =
                CreateBinFileAnalysis.buildChannelIdentityGrid(draft, true, true, null);
        grid.nameFields[0].setText("DAPI");
        grid.nameFields[1].setText("IBA1");
        grid.nameFields[2].setText("My free-form marker");

        Object bindings = newBinSetupBindings(3);
        copyBindingArray(bindings, "nameFields", grid.nameFields);
        copyBindingArray(bindings, "colorCombos", grid.lutCombos);

        CreateBinFileAnalysis.BinUserConfig result =
                invokeBuildBinUserConfigFromDialog(analysis, 3, draft, bindings);

        assertEquals(Arrays.asList("nuclei_dapi", "microglia_iba1", ""),
                result.markerIds);
        assertEquals(Arrays.asList("round", "complex", ""), result.markerShapes);
        assertEquals(Arrays.asList(Boolean.TRUE, Boolean.TRUE, Boolean.FALSE),
                result.markerCrowdingSensitive);

        File settings = configurationDir(temp.newFolder("production-marker-binding"));
        analysis.writeBinConfigFiles(settings, result);
        ChannelConfig raw = ChannelConfigIO.read(settings);
        assertEquals("round", raw.channels.get(0).markerShape);
        assertTrue(raw.channels.get(0).markerCrowdingSensitive);
        assertEquals("complex", raw.channels.get(1).markerShape);
        assertTrue(raw.channels.get(1).markerCrowdingSensitive);
        assertEquals("", raw.channels.get(2).markerId);
        ChannelIdentities identities = ChannelConfigIO.toChannelIdentities(raw);
        assertEquals("nuclei_dapi", identities.findByChannelIndex(0).getMarkerId());
        assertEquals("microglia_iba1", identities.findByChannelIndex(1).getMarkerId());
        assertNull(identities.findByChannelIndex(2));
    }

    @Test
    public void productionIdentityExtractionRejectsMissingSavedMarkerId() throws Exception {
        CreateBinFileAnalysis analysis = new CreateBinFileAnalysis();
        CreateBinFileAnalysis.BinUserConfig draft = twoChannelConfig();
        draft.names.set(0, "Retired free-form marker");
        draft.markerIds.set(0, "missing_library_marker");
        CreateBinFileAnalysis.ChannelIdentityGrid grid =
                CreateBinFileAnalysis.buildChannelIdentityGrid(draft, true, true, null);
        Object bindings = newBinSetupBindings(2);
        copyBindingArray(bindings, "nameFields", grid.nameFields);
        ...
        try {
            invokeBuildBinUserConfigFromDialog(analysis, 2, draft, bindings);
            assertTrue("Expected stale marker identity to be rejected.", false);
        } catch (java.lang.reflect.InvocationTargetException e) {
            assertTrue(e.getCause() instanceof IllegalArgumentException);
            assertTrue(e.getCause().getMessage().contains("missing_library_marker"));
        }
    }

    @Test
    public void editingRetiredSavedMarkerToFreeFormDiscardsStaleIdentity() throws Exception {
        CreateBinFileAnalysis analysis = new CreateBinFileAnalysis();
        CreateBinFileAnalysis.BinUserConfig draft = twoChannelConfig();
        draft.names.set(0, "Retired marker label");
        draft.markerIds.set(0, "missing_library_marker");
        CreateBinFileAnalysis.ChannelIdentityGrid grid =
                CreateBinFileAnalysis.buildChannelIdentityGrid(draft, true, true, null);
        grid.nameFields[0].setText("My replacement free-form target");
        Object bindings = newBinSetupBindings(2);
        copyBindingArray(bindings, "nameFields", grid.nameFields);
        copyBindingArray(bindings, "colorCombos", grid.lutCombos);
        ...
        assertEquals("", result.markerIds.get(0));
        assertEquals("", result.markerShapes.get(0));
        assertFalse(result.markerCrowdingSensitive.get(0).booleanValue());
    }

    @Test
    public void productionIdentityExtractionPreservesUnchangedCopiedMarkerAndClearsItWhenRenamed()
            throws Exception {
        CreateBinFileAnalysis analysis = new CreateBinFileAnalysis();
        CreateBinFileAnalysis.BinUserConfig draft = twoChannelConfig();
        draft.names.set(0, "My nuclear display label");
        draft.markerIds.set(0, "nuclei_dapi");
        draft.markerShapes.set(0, "round");
        draft.markerCrowdingSensitive.set(0, Boolean.TRUE);
        CreateBinFileAnalysis.ChannelIdentityGrid grid =
                CreateBinFileAnalysis.buildChannelIdentityGrid(draft, true, true, null);
        Object bindings = newBinSetupBindings(2);
        copyBindingArray(bindings, "nameFields", grid.nameFields);
        ...
        CreateBinFileAnalysis.BinUserConfig preserved =
                invokeBuildBinUserConfigFromDialog(analysis, 2, draft, bindings);
        assertEquals("nuclei_dapi", preserved.markerIds.get(0));
        assertEquals("round", preserved.markerShapes.get(0));
        assertTrue(preserved.markerCrowdingSensitive.get(0).booleanValue());

        grid.nameFields[0].setText("A genuinely free-form replacement");
        CreateBinFileAnalysis.BinUserConfig unbound =
                invokeBuildBinUserConfigFromDialog(analysis, 2, draft, bindings);
        assertEquals("", unbound.markerIds.get(0));
        assertEquals("", unbound.markerShapes.get(0));
        assertFalse(unbound.markerCrowdingSensitive.get(0).booleanValue());
    }

    @Test
    public void productionIdentityExtractionKeepsGreekMarkersAndStableIdsDistinct() throws Exception {
        CreateBinFileAnalysis analysis = new CreateBinFileAnalysis();
        CreateBinFileAnalysis.BinUserConfig draft = twoChannelConfig();
        CreateBinFileAnalysis.ChannelIdentityGrid grid =
                CreateBinFileAnalysis.buildChannelIdentityGrid(draft, true, true, null);
        grid.nameFields[0].setText("PDGFRα");
        grid.nameFields[1].setText("PDGFRβ");
        Object bindings = newBinSetupBindings(2);
        copyBindingArray(bindings, "nameFields", grid.nameFields);
        copyBindingArray(bindings, "colorCombos", grid.lutCombos);
        ...
        assertEquals(Arrays.asList("opc_pdgfra", "pericytes_pdgfrb"), result.markerIds);
    }

    @Test
    public void headlessAmbiguousMarkerFailsWithEveryStableCandidate() throws Exception {
        CreateBinFileAnalysis analysis = new CreateBinFileAnalysis();
        analysis.setHeadless(true);
        CreateBinFileAnalysis.BinUserConfig draft = twoChannelConfig();
        CreateBinFileAnalysis.ChannelIdentityGrid grid =
                CreateBinFileAnalysis.buildChannelIdentityGrid(draft, true, true, null);
        grid.nameFields[0].setText("CR");
        Object bindings = newBinSetupBindings(2);
        copyBindingArray(bindings, "nameFields", grid.nameFields);
        copyBindingArray(bindings, "colorCombos", grid.lutCombos);
        ...
        try {
            invokeBuildBinUserConfigFromDialog(analysis, 2, draft, bindings);
            assertTrue("Expected ambiguous headless marker input to fail.", false);
        } catch (java.lang.reflect.InvocationTargetException expected) {
            String message = expected.getCause().getMessage();
            assertTrue(message.contains("amyloid_congo_red (Congo Red)"));
            assertTrue(message.contains("interneurons_cr (Calretinin)"));
            assertTrue(message.contains("Supply one of the stable marker IDs explicitly"));
        }
    }

    @Test
    public void guiAmbiguousMarkerRequiresExplicitCandidateChoice() throws Exception {
        MarkerChoiceAnalysis analysis = new MarkerChoiceAnalysis("interneurons_cr");
        CreateBinFileAnalysis.BinUserConfig draft = twoChannelConfig();
        CreateBinFileAnalysis.ChannelIdentityGrid grid =
                CreateBinFileAnalysis.buildChannelIdentityGrid(draft, true, true, null);
        grid.nameFields[0].setText("CR");
        ...
        assertEquals(Arrays.asList("amyloid_congo_red", "interneurons_cr"),
                analysis.offeredStableIds);
        assertEquals("interneurons_cr", result.markerIds.get(0));
        assertEquals("complex", result.markerShapes.get(0));
        assertTrue(result.markerCrowdingSensitive.get(0).booleanValue());
    }

    @Test
    public void acceptedStableIdPrecedesFreeFormAndPersistsIdentity() throws Exception {
        CreateBinFileAnalysis analysis = new CreateBinFileAnalysis();
        analysis.setHeadless(true);
        CreateBinFileAnalysis.BinUserConfig draft = twoChannelConfig();
        CreateBinFileAnalysis.ChannelIdentityGrid grid =
                CreateBinFileAnalysis.buildChannelIdentityGrid(draft, true, true, null);
        grid.nameFields[0].setText("reporter_mcherry");
        grid.nameFields[1].setText("My unregistered target");
        Object bindings = newBinSetupBindings(2);
        copyBindingArray(bindings, "nameFields", grid.nameFields);
        copyBindingArray(bindings, "colorCombos", grid.lutCombos);
        ...
        assertEquals("reporter_mcherry", result.markerIds.get(0));
        assertEquals("", result.markerIds.get(1));

        File settings = configurationDir(temp.newFolder("stable-marker-id-roundtrip"));
        analysis.writeBinConfigFiles(settings, result);
        ChannelConfig reloaded = ChannelConfigIO.read(settings);
        assertEquals("reporter_mcherry", reloaded.channels.get(0).markerId);
        assertEquals("", reloaded.channels.get(1).markerId);
    }

    @Test
    public void headlessCliRenameDoesNotRetainPresetMarkerIdentity() throws Exception {
        File project = temp.newFolder("headless-marker-rename");
        File input = new File(project, "input");
        assertTrue(input.mkdirs());
        assertTrue(new FileSaver(new ImagePlus("source", new ByteProcessor(2, 2)))
                .saveAsTiff(new File(input, "source.tif").getAbsolutePath()));

        BinConfig payload = new BinConfig();
        payload.channelNames.add("DAPI");
        payload.channelColors.add("Blue");
        payload.channelThresholds.add("default");
        payload.channelSizes.add("100-Infinity");
        payload.channelMinMax.add("None");
        payload.channelIntensityThresholds.add("default");
        payload.channelFilterPresets.add("Default");
        payload.segmentationMethods.add("classical");
        BinPreset preset = new BinPreset("DAPI preset", "literal test", "1", payload,
                Collections.singletonList("nuclei_dapi"),
                Collections.singletonList("round"),
                Collections.singletonList(Boolean.TRUE));
        CLIConfig cli = new CLIConfig();
        cli.getBin().getNames().put(Integer.valueOf(0), "My replacement free-form target");
        CreateBinFileAnalysis analysis = new CreateBinFileAnalysis();
        analysis.setHeadless(true);
        ...
        ChannelConfig written = ChannelConfigIO.read(configurationDir(project));
        assertEquals("My replacement free-form target", written.channels.get(0).name);
        assertEquals("", written.channels.get(0).markerId);
        assertEquals("", written.channels.get(0).markerShape);
        assertFalse(written.channels.get(0).markerCrowdingSensitive);
    }
    ...
    public void buildConfigFromDialogUsesDraftAndAppliedHiddenChannelMetadata() throws Exception {
        ...
        applied.markerIds.addAll(Arrays.asList("neurons_neun", "microglia_iba1"));
        ...
        assertEquals(Arrays.asList("neurons_neun", "microglia_iba1"), result.markerIds);
        assertEquals(Arrays.asList("round", "complex"), result.markerShapes);
        assertEquals(Arrays.asList(Boolean.TRUE, Boolean.TRUE), result.markerCrowdingSensitive);
    ...
    private static class RecordingFilteredAnalysis extends CreateBinFileAnalysis {
        ...
        protected BinUserConfig collectBinConfigFromUser(String directory, File binFolder,
            ...
            c.markerIds.clear();
            c.markerIds.addAll(Arrays.asList("neurons_neun", "nuclei_dapi"));
            c.markerShapes.clear();
            c.markerShapes.addAll(Arrays.asList("round", "round"));
            c.markerCrowdingSensitive.clear();
            c.markerCrowdingSensitive.addAll(Arrays.asList(Boolean.TRUE, Boolean.TRUE));
            return c;
        }
    ...
    private static final class FullResetAnalysis extends CreateBinFileAnalysis {
        enum Mode { COMPLETE, DISCARD, SAVE_DRAFT, FAIL }

        private final Mode mode;
        boolean receivedNullExisting;

        FullResetAnalysis(Mode mode) {
            this.mode = mode;
        }

        @Override
        void handleFullCreation(String directory, File binFolder, BinConfig existing,
                                ResumeDecision resumeDecision) throws java.io.IOException {
            receivedNullExisting = existing == null;
            if (mode == Mode.FAIL) {
                throw new java.io.IOException("injected full reset failure");
            }
            BinUserConfig fresh = threeChannelConfig();
            fresh.names.set(0, "Fresh DAPI");
            fresh.names.set(1, "Fresh IBA1");
            fresh.names.set(2, "Fresh GFAP");
            if (mode == Mode.COMPLETE) {
                ChannelConfig complete = ChannelConfigIO.fromBinUserConfig(fresh);
                complete.complete = Boolean.TRUE;
                ChannelConfigIO.write(binFolder, complete);
                return;
            }
            CancelConfirmationDialog.Choice expectedChoice = mode == Mode.SAVE_DRAFT
                    ? CancelConfirmationDialog.Choice.SAVE_AND_EXIT
                    : CancelConfirmationDialog.Choice.DISCARD_AND_EXIT;
            if (expectedChoice == CancelConfirmationDialog.Choice.SAVE_AND_EXIT) {
                assertTrue(handleCancelRequest(null, binFolder, fresh, new boolean[7][3],
                        1, "Channel Identity", null));
            } else {
                assertTrue(handleCancelRequest(null, binFolder, fresh, null,
                        1, "Channel Identity", null));
            }
        }
        ...
        @Override
        protected CancelConfirmationDialog.Choice showCancelConfirmation(
                Window owner, String stepLabel, List<String> progressLines,
                String draftPath) {
            return mode == Mode.SAVE_DRAFT
                    ? CancelConfirmationDialog.Choice.SAVE_AND_EXIT
                    : CancelConfirmationDialog.Choice.DISCARD_AND_EXIT;
        }
    }

    private static final class MarkerChoiceAnalysis extends CreateBinFileAnalysis {
        private final String selectedStableId;
        final List<String> offeredStableIds = new ArrayList<String>();

        MarkerChoiceAnalysis(String selectedStableId) {
            this.selectedStableId = selectedStableId;
        }

        @Override
        protected MarkerLibrary.Entry chooseAmbiguousMarker(
                String acceptedText, List<MarkerLibrary.Entry> candidates) {
            offeredStableIds.clear();
            for (MarkerLibrary.Entry candidate : candidates) {
                offeredStableIds.add(candidate.getId());
                if (selectedStableId.equals(candidate.getId())) return candidate;
            }
            throw new IllegalArgumentException("Test candidate not offered: " + selectedStableId);
        }
    }
}
