...
public class AnalysisHelpDialogTest {
    ...
    public void missingImageResourceRendersPlaceholder() {
        AnalysisHelpTopic topic = new AnalysisHelpTopic(
                ...
                "Test Topic",
                "What it does",
                Collections.singletonList("Needs first"),
                Collections.singletonList("Produces"),
                Arrays.asList("First", "Second"),
                Collections.singletonList("Watch out"),
    ...
    @Test
    public void spatialConceptImagesStackVerticallyNotInAGrid() {
        JPanel panel = AnalysisHelpDialog.buildContentPanel(
                AnalysisHelpCatalog.forAnalysis(FLASH_Pipeline.IDX_SPATIAL));

        // The concept diagrams are rendered as full-size HelpImagePanels...
        assertTrue(containsComponent(panel, HelpImagePanel.class));
        // ...stacked vertically, not laid out in a multi-column gallery grid.
        assertTrue("concept images should not be placed in a grid gallery",
                findImageGallery(panel) == null);
    }
