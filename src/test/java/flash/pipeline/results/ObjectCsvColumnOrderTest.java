...
public class ObjectCsvColumnOrderTest {
    ...
    @Test
    public void overlapCountColumnsSortWithCpcAfterContains() {
        // Marker-centroid object counting lives in the CPC section (7), right after CPCContains and
        // before the bounding-box coloc section (8).
        List<String> input = Arrays.asList(
                "Label",
                "C1_IsCluster_C2",
                "C1_BBColoc_C2",        // section 8 (later)
                "C1_CPCContains_C2",
                "C1_OverlapCount_C2",
                "C1_HasMarker_C2",
                "C1_CPCColoc_C2");
        ...
        int cpcColoc = ordered.indexOf("C1_CPCColoc_C2");
        int cpcContains = ordered.indexOf("C1_CPCContains_C2");
        int overlap = ordered.indexOf("C1_OverlapCount_C2");
        int hasMarker = ordered.indexOf("C1_HasMarker_C2");
        int isCluster = ordered.indexOf("C1_IsCluster_C2");
        int bbColoc = ordered.indexOf("C1_BBColoc_C2");

        // CPC section internal order: coloc, contains, then overlap count + its derived flags.
        assertTrue(cpcColoc < cpcContains);
        assertTrue(cpcContains < overlap);
        assertTrue(overlap < hasMarker);
        assertTrue(hasMarker < isCluster);
        // Whole CPC section precedes the bounding-box coloc section.
        assertTrue(isCluster < bbColoc);
    }
