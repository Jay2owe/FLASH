...
public class RoiOrientationManifestServiceTest {
    ...
    @Test
    public void identityForProjectTiffKeepsInputRelativePathAndStableKey() throws Exception {
        File root = temp.newFolder("saved-tiff-project");
        File input = new File(root, "input");
        assertTrue(input.mkdirs());
        File tiff = new File(input, "B_Mouse_RH_SCN.tif");
        assertTrue(tiff.createNewFile());

        OrientationImageIdentity beforeSave = OrientationImageIdentity.fromProjectSeries(
                root.getAbsolutePath(), 0, "input - B_Mouse_RH_SCN");

        ProjectFile project = new ProjectFile();
        project.name = "Renamed Saved Project";
        ProjectFile.Item item = new ProjectFile.Item();
        item.path = tiff.getAbsolutePath();
        item.include = true;
        project.items.add(item);
        ProjectFileIO.write(
                FlashProjectLayout.forDirectory(root.getAbsolutePath()).configurationWriteDir(),
                project);

        OrientationImageIdentity afterSave = OrientationImageIdentity.fromProjectSeries(
                root.getAbsolutePath(), 0, "Renamed Saved Project - B_Mouse_RH_SCN");

        assertEquals("input/B_Mouse_RH_SCN.tif", beforeSave.sourceFile);
        assertEquals("input/B_Mouse_RH_SCN.tif", afterSave.sourceFile);
        assertEquals(beforeSave.imageKey, afterSave.imageKey);
        assertEquals(
                OrientationManifestRow.buildImageKey(
                        "TIFF",
                        "input/B_Mouse_RH_SCN.tif",
