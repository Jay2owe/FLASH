...
import flash.pipeline.io.CsvSupportTest.FileOperationFaults;
import ij.ImagePlus;
...
public class SpectralOutputWriterTest {
    ...
    @Test
    public void multiArtifactFaultsPreserveEveryPriorGeneration() throws Exception {
        File directory = temp.newFolder("spectral-publication-faults");
        SpectralOutputWriter.RunMetadata metadata = runMetadata();
        ...
        summaryRows.add(SpectralOutputWriter.buildPerImageSummaryRow(
                ...
                "Linear unmixing",
                "processed",
                new File(directory, "corrected_Target.tif"),
                null,
                "Full stack",
                new ArrayList<CorrectionPipeline.FeatureSummary>(),
                "",
        ...
        List<CorrectionPipeline.FeatureSummary> features =
                new ArrayList<CorrectionPipeline.FeatureSummary>();
        features.add(new CorrectionPipeline.FeatureSummary("linear_unmixing", "Linear unmixing")
                .putDouble("weight_channel_2", 0.25));
        List<Map<String, String>> coefficientRows =
                SpectralOutputWriter.buildCoefficientRows(
                        0, "Mouse1_LH_SCN", "Control", "control", metadata,
                        "processed", features);

        SpectralOutputWriter.writePerImageSummary(directory.getAbsolutePath(), summaryRows);
        SpectralOutputWriter.writeCorrectionCoefficients(
                directory.getAbsolutePath(), coefficientRows);

        File summary = SpectralOutputWriter.perImageSummaryFile(directory.getAbsolutePath());
        File coefficients =
                SpectralOutputWriter.correctionCoefficientsFile(directory.getAbsolutePath());
        List<File> targets = Arrays.asList(summary, coefficients);
        List<byte[]> replacements = Arrays.asList(
                "SeriesIndex,RunAction\n0,reprocessed\n".getBytes(StandardCharsets.UTF_8),
                "SeriesIndex,Metric,Value\n0,weight_channel_2,0.500000\n"
                        .getBytes(StandardCharsets.UTF_8));

        for (FileOperationFaults.Trigger trigger : Arrays.asList(
                FileOperationFaults.Trigger.TRUNCATED_OUTPUT,
                FileOperationFaults.Trigger.REOPEN_VALIDATION)) {
            FileOperationFaults.FailureObservation observation =
                    FileOperationFaults.failBatchPublication(
                            targets, replacements, 1, trigger);

            assertEquals(trigger, observation.getTrigger());
            assertEquals(coefficients.toPath().toAbsolutePath().normalize(),
                    observation.getAffectedPath());
            assertTrue(observation.getFailureMessage().contains(trigger.getLabel()));
            assertTrue(observation.getFailureMessage().contains(coefficients.getAbsolutePath()));
            assertEquals(2, observation.getTargetCount());
            for (int i = 0; i < observation.getTargetCount(); i++) {
                assertArrayEquals("Every artifact in the prior spectral generation must survive "
                                + trigger.getLabel(),
                        observation.getPriorGeneration(i),
                        observation.getCurrentGeneration(i));
                assertArrayEquals(observation.getPriorGeneration(i),
                        Files.readAllBytes(targets.get(i).toPath()));
            }
            assertTrue("Batch staging files must be cleaned for " + trigger.getLabel(),
                    observation.isStagingClean());
        }

        assertTrue(SpectralOutputWriter.readPerImageSummaryRows(directory.getAbsolutePath())
                .containsKey(Integer.valueOf(0)));
        assertTrue(SpectralOutputWriter.readCoefficientRows(directory.getAbsolutePath())
                .containsKey(Integer.valueOf(0)));
    }
