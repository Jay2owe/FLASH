...
public class SyncedSliceControllerTest {
    ...
    @Test
    public void pendingCellsUseSourceStackDepthForClamp() throws Exception {
        final SyncedSliceController controller = new SyncedSliceController();
        final VariationCellPanel pending = pendingCellWithSourceSlices(5);
        SwingUtilities.invokeAndWait(new Runnable() {
            @Override public void run() {
                controller.register(pending);
        ...
        SwingUtilities.invokeAndWait(new Runnable() {
            @Override public void run() {
                pending.preview().setImage(stack(5));
                controller.setSlice(controller.currentSlice());
            }
        });
        assertEquals(4, pending.currentZForTest());
    }
    ...
    private static VariationCellPanel pendingCellWithSourceSlices(final int slices)
            throws Exception {
        final AtomicReference<VariationCellPanel> ref =
                new AtomicReference<VariationCellPanel>();
        SwingUtilities.invokeAndWait(new Runnable() {
            @Override public void run() {
                VariationCellPanel cell = new VariationCellPanel(
                        ParameterCombo.builder().build(), stack(slices), null, null);
                ref.set(cell);
            }
        });
        return ref.get();
