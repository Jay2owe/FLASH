...
public class ImageSourceDispatcherTest {
    ...
    @Test
    public void modeAfterLooseTiffRelocationChoice_cancelledAborts() throws Exception {
        File dir = temp.newFolder("loose-tiffs-cancel");
        new File(dir, "alpha.tif").createNewFile();

        try {
            ImageSourceDispatcher.modeAfterLooseTiffRelocationChoice(
                    dir.getAbsolutePath(),
                    ImageSourceDispatcher.SourceMode.TIFF_LOOSE,
                    LooseTiffRelocator.Choice.CANCELLED);
            fail("Expected cancellation to abort loose TIFF source creation.");
        } catch (IllegalArgumentException e) {
