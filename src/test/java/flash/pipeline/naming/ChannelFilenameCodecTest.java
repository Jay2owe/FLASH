...
public class ChannelFilenameCodecTest {
    ...
    @Test
    public void windowsControlsAreEncodedAndRoundTrip() {
        String raw = "DAPI\u0000\t\n\u001fend";
        String safe = ChannelFilenameCodec.toSafe(raw);
        assertEquals("DAPI%00%09%0A%1Fend", safe);
        assertFalse(ChannelFilenameCodec.isSafe(raw));
        assertEquals(raw, ChannelFilenameCodec.toRaw(safe));
    }

    @Test
    public void nonAsciiControlUsesUtf8PercentBytes() {
        String raw = "A\u0085B";
        assertEquals("A%C2%85B", ChannelFilenameCodec.toSafe(raw));
        assertEquals(raw, ChannelFilenameCodec.toRaw(ChannelFilenameCodec.toSafe(raw)));
        assertFalse(ChannelFilenameCodec.isSafe(raw));
    }

    @Test
    public void validUnicodeIncludingSupplementaryCodePointsPassesThrough() {
        String raw = "DAPI-处理-α-\uD83E\uDDEC";
        assertEquals(raw, ChannelFilenameCodec.toSafe(raw));
        assertTrue(ChannelFilenameCodec.isSafe(raw));
        assertRoundTrip(raw);
    }

    @Test
    public void isolatedSurrogatesAreEncodedLosslessly() {
        String raw = "bad\uD800name";
        assertEquals("bad%uD800name", ChannelFilenameCodec.toSafe(raw));
        assertFalse(ChannelFilenameCodec.isSafe(raw));
        assertEquals(raw, ChannelFilenameCodec.toRaw(ChannelFilenameCodec.toSafe(raw)));
    }

    @Test
    public void malformedExternalPercentSequencesAreNotCorrupted() {
        assertEquals("A%ZZB", ChannelFilenameCodec.toRaw("A%ZZB"));
        assertEquals("A%C2%41B", ChannelFilenameCodec.toRaw("A%C2%41B"));
        assertEquals("A%uXYZWB", ChannelFilenameCodec.toRaw("A%uXYZWB"));
    }
