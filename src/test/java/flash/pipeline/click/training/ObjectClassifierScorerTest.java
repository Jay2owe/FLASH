...
public class ObjectClassifierScorerTest {
    ...
    @Test
    public void filterLabelImageDoesNotExposeCopiedStarDistRowsAfterRelabeling() {
        ImagePlus labels = labelsWithValues(2, 5, 5, 0, 2);
        ResultsTable sourceStats = starDistStats(2, 5);
        labels.setProperty(StarDist3DRunner.OBJECT_STATS_PROPERTY, sourceStats);
        List<ObjectClassifierScorer.ScoreResult> results = Arrays.asList(
                new ObjectClassifierScorer.ScoreResult(2, 0.95, false),
                new ObjectClassifierScorer.ScoreResult(5, 0.91, true));

        ImagePlus filtered = new ObjectClassifierScorer().filterLabelImage(labels, results);

        assertEquals(Collections.singleton(Integer.valueOf(1)), labelsIn(filtered));
        assertNull("strict StarDist consumers must not see source-label rows on RF labels",
                LabelIndex.starDistStats(filtered));
        assertSame("source metadata remains owned by the source image", sourceStats,
                LabelIndex.starDistStats(labels));
    }

    @Test
    public void filterAndScoreTableKeepExactHighSourceIdentities() {
        ImagePlus labels = labelsWithFloatValues(65_535, 65_536, 70_000, 0);
        List<ObjectClassifierScorer.ScoreResult> results = Arrays.asList(
                new ObjectClassifierScorer.ScoreResult(70_000, 0.99, true),
                new ObjectClassifierScorer.ScoreResult(65_535, 0.10, false),
                new ObjectClassifierScorer.ScoreResult(65_536, 0.98, true));
        ObjectClassifierScorer scorer = new ObjectClassifierScorer();

        ImagePlus filtered = scorer.filterLabelImage(labels, results);
        ResultsTable table = scorer.toResultsTable(results);

        assertEquals(0, Math.round(filtered.getProcessor().getf(0, 0)));
        assertEquals(1, Math.round(filtered.getProcessor().getf(1, 0)));
        assertEquals(2, Math.round(filtered.getProcessor().getf(2, 0)));
        assertEquals(70_000, (int) table.getValue("Label", 0));
        assertEquals(2, (int) table.getValue("NewLabel", 0));
        assertEquals(65_535, (int) table.getValue("Label", 1));
        assertEquals(0, (int) table.getValue("NewLabel", 1));
        assertEquals(65_536, (int) table.getValue("Label", 2));
        assertEquals(1, (int) table.getValue("NewLabel", 2));
    }
    ...
    private static ImagePlus labelsWithFloatValues(int... values) {
        FloatProcessor fp = new FloatProcessor(values.length, 1);
        for (int i = 0; i < values.length; i++) {
            fp.setf(i, 0, values[i]);
        }
        ImageStack stack = new ImageStack(values.length, 1);
        stack.addSlice(fp);
        return new ImagePlus("float labels", stack);
    }

    private static ResultsTable starDistStats(int... labels) {
        ResultsTable table = new ResultsTable();
        for (int row = 0; row < labels.length; row++) {
            table.incrementCounter();
            table.setValue("Label", row, labels[row]);
            table.setValue(StarDist3DRunner.STATS_QUALITY_MEAN, row, labels[row] / 10.0);
        }
        return table;
    }
