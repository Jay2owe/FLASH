...
public class CellposeDatasetPackagerTest {
    ...
    @Test
    public void matchingMultiFramePairFailsPreflightBeforeDatasetArtifacts() throws Exception {
        Path root = projectRoot();
        ClickStore store = new ClickStore();
        store.add(click("Timed", 1, 1, ClickStore.Verdict.POSITIVE));
        ImagePlus raw = rawStack(2, 1, 2);
        raw.setDimensions(1, 1, 2);
        raw.setOpenAsHyperStack(true);
        ImagePlus labels = labelStack(2, 1, new int[][][] {
                { {0, 0, 1} },
                { {1, 0, 1} }
        });
        labels.setDimensions(1, 1, 2);
        labels.setOpenAsHyperStack(true);
        ...
        try {
            ...
            assertTrue(expected.getMessage(), expected.getMessage().contains("T=2"));
            assertTrue(expected.getMessage(), expected.getMessage().contains("expected T=1"));
        }

        assertFalse("Geometry preflight must not create Cellpose dataset artifacts",
                Files.exists(engineRoot));
    }

    @Test
    public void multiChannelLabelImageFailsPreflightBeforeDatasetArtifacts() throws Exception {
        Path root = projectRoot();
        ClickStore store = new ClickStore();
        store.add(click("MultiLabel", 1, 1, ClickStore.Verdict.POSITIVE));
        ImagePlus labels = labelStack(2, 1, new int[][][] {
                { {0, 0, 1} },
                { {1, 0, 2} }
        });
        ...
        try {
            ...
            assertTrue(expected.getMessage(), expected.getMessage().contains("C=2"));
            assertTrue(expected.getMessage(), expected.getMessage().contains("expected C=1"));
        }
    ...
    @Test
    public void wideCanonicalLabelsRemainExactInReopenedCellposeMask() throws Exception {
        Path root = projectRoot();
        ...
        ImagePlus mask = IJ.openImage(result.outputDir.resolve(
                "Wide_C1_z001_masks.tif").toString());
        try {
            assertNotNull("Wide Cellpose mask must reopen", mask);
            assertEquals("Labels above 65,535 require lossless 32-bit storage",
                    32, mask.getBitDepth());
            for (int x = 0; x < oracle.length; x++) {
                assertEquals(oracle[x], (int) mask.getProcessor().getf(x, 0));
            }
        } finally {
            close(mask);
        }
    ...
    @Test
    public void lateInvalidLabelFailsBeforeAnyDatasetDirectoryOrRawFileExists()
            throws Exception {
        Path root = projectRoot();
        ClickStore store = new ClickStore();
        store.add(click("First", 1, 65_535, ClickStore.Verdict.POSITIVE));
        store.add(click("Second", 1, 70_000, ClickStore.Verdict.POSITIVE));
        Map<String, ImagePlus> raw = new LinkedHashMap<String, ImagePlus>();
        raw.put("First", rawStack(2, 1, 1));
        raw.put("Second", rawStack(2, 1, 1));
        Map<String, ImagePlus> labels = new LinkedHashMap<String, ImagePlus>();
        labels.put("First", wideLabelImage("first", new int[] {65_535, 0}));
        FloatProcessor invalid = new FloatProcessor(2, 1);
        invalid.setf(0, 70_000.5f);
        labels.put("Second", image("second", invalid));
        Path engineRoot = FlashProjectLayout.forDirectory(root.toString())
                .trainingDatasetsRoot().toPath().resolve("Cellpose");
        ...
        try {
            ...
            assertTrue(expected.getMessage(), expected.getMessage().contains("second"));
        }
    ...
    public void unrepresentableClickLabelFailsBeforeProviderOrDirectoryUse() throws Exception {
        Path root = projectRoot();
        ClickStore store = new ClickStore();
        store.add(click("TooWide", 1, 16_777_217, ClickStore.Verdict.POSITIVE));
        ...
        try {
            new CellposeDatasetPackager().packageDataset(root, "too-wide", 1, store,
                    provider, provider, "cyto3");
    ...
    private static ImagePlusProvider provider(final Map<String, ImagePlus> images) {
        return new ImagePlusProvider() {
            @Override
            public ImagePlus get(String requested) {
                return images.get(requested);
            }
        };
    }
    ...
    private static ImagePlus wideLabelImage(String title, int[] values) {
        ...
        return image(title, processor);
    }

    private static ImagePlus image(String title, ij.process.ImageProcessor processor) {
