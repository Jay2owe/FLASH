...
public class ParametricStatisticsTest {
    ...
    @Test
    public void welchTTest_unequalConstantsAreDeterministicallySeparated() {
        double[] result = ParametricStatistics.welchTTest(
    ...
    @Test
    public void pairedAndRepeatedConstantEffectsAreDeterministicallySeparated() {
        double[] paired = PairedTests.pairedTTest(
                Arrays.asList(1.0, 1.0, 1.0),
                Arrays.asList(2.0, 2.0, 2.0));
        assertEquals(Double.NEGATIVE_INFINITY, paired[0], 0.0);
        assertEquals(0.0, paired[1], 0.0);

        List<List<Double>> separated = new ArrayList<List<Double>>();
        separated.add(Arrays.asList(1.0, 1.0, 1.0));
        separated.add(Arrays.asList(2.0, 2.0, 2.0));
        separated.add(Arrays.asList(3.0, 3.0, 3.0));
        double[] repeated = PairedTests.repeatedMeasuresAnova(separated);
        assertEquals(Double.POSITIVE_INFINITY, repeated[0], 0.0);
        assertEquals(0.0, repeated[1], 0.0);

        List<List<Double>> parallel = new ArrayList<List<Double>>();
        parallel.add(Arrays.asList(1.0, 2.0, 4.0));
        parallel.add(Arrays.asList(2.0, 3.0, 5.0));
        parallel.add(Arrays.asList(4.0, 5.0, 7.0));
        assertArrayEquals(new double[]{Double.POSITIVE_INFINITY, 0.0},
                PairedTests.repeatedMeasuresAnova(parallel), 0.0);

        List<List<Double>> equal = new ArrayList<List<Double>>();
        equal.add(Arrays.asList(5.0, 5.0, 5.0));
        equal.add(Arrays.asList(5.0, 5.0, 5.0));
        assertArrayEquals(new double[]{0.0, 1.0},
                PairedTests.repeatedMeasuresAnova(equal), 0.0);
    }

    @Test
    public void tukeyRetainsZeroVarianceComparisons() {
        List<List<Double>> groups = new ArrayList<List<Double>>();
        groups.add(Arrays.asList(1.0, 1.0, 1.0));
        groups.add(Arrays.asList(2.0, 2.0, 2.0));
        groups.add(Arrays.asList(2.0, 2.0, 2.0));

        List<PostHocRow> rows = TukeyHSD.pairwise(
                groups, Arrays.asList("A", "B", "C"));

        assertEquals(3, rows.size());
        assertEquals(Double.POSITIVE_INFINITY, find(rows, "A", "B").statistic, 0.0);
        assertEquals(0.0, find(rows, "A", "B").pAdjusted, 0.0);
        assertEquals(0.0, find(rows, "B", "C").statistic, 0.0);
        assertEquals(1.0, find(rows, "B", "C").pAdjusted, 0.0);
    }
    ...
    private static PostHocRow find(List<PostHocRow> rows, String a, String b) {
        for (PostHocRow row : rows) {
            if (a.equals(row.groupA) && b.equals(row.groupB)) return row;
        }
